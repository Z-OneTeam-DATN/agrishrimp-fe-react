...
import { Eye, Building2, Phone, ChevronLeft, ChevronRight, BadgeCheck, CreditCard, Percent } from "lucide-react";
    ...
    <div className="w-full">
      <Table className="table-custom border-collapse min-w-[1100px]">
        <TableHeader>
          <TableRow className="bg-[#f0f0f0] hover:bg-[#f0f0f0] border-b border-[#ccc]">
            <TableHead className="w-[80px] font-bold text-[#1f1f1f] text-[11px] uppercase p-2 pl-4">Mã ĐL</TableHead>
            <TableHead className="font-bold text-[#1f1f1f] text-[11px] uppercase p-2">Đại lý & Mã số thuế</TableHead>
            <TableHead className="w-[120px] font-bold text-[#1f1f1f] text-[11px] uppercase p-2">Phân cấp</TableHead>
            <TableHead className="w-[150px] font-bold text-[#1f1f1f] text-[11px] uppercase p-2">Người đại diện</TableHead>
            <TableHead className="w-[150px] text-right font-bold text-[#1f1f1f] text-[11px] uppercase p-2">Công nợ (₫)</TableHead>
            <TableHead className="w-[100px] text-center font-bold text-[#1f1f1f] text-[11px] uppercase p-2">Chiết khấu</TableHead>
        ...
        <TableBody>
          {dealers.map((dl) => (
            <TableRow key={dl.id} className="hover:bg-[#f0f8ff] border-b border-[#eee] transition-colors cursor-pointer">
              <TableCell className="p-2 pl-4 text-[12px] font-bold text-slate-500">#{dl.id}</TableCell>
              <TableCell className="p-2">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[13px] font-bold text-slate-800 uppercase tracking-tighter">{dl.name}</span>
                    <span className="text-[10px] text-slate-400 font-mono">MST: {dl.taxCode}</span>
                  </div>
                </div>
              </TableCell>
              <TableCell className="p-2">
              ...
              <TableCell className="p-2">
                <div className="flex flex-col">
                  <span className="text-[12px] font-bold text-slate-700">{dl.representative}</span>
                  <span className="text-[10px] text-slate-400 font-medium">{dl.phone}</span>
                </div>
              </TableCell>
              <TableCell className="p-2 text-right">
                <div className="flex flex-col items-end">
                  <span className="text-[12px] font-bold text-rose-600">{dl.debt}</span>
                  <span className="text-[9px] text-slate-400 font-bold uppercase">Hạn mức vượt: 0₫</span>
                </div>
              </TableCell>
              ...
              <TableCell className="p-2">
                <span className={cn(
                  ...
                  dl.status === "Hoạt động" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-slate-100 text-slate-400 border-slate-200"
                )}>
                  {dl.status}
                </span>
              </TableCell>
      ...
      <div className="flex items-center justify-between px-3 py-2 border-t border-[#eee] bg-[#f8f9fa]">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button variant="outline" size="sm" className="h-6 w-6 p-0 text-[10px] bg-amber-500 text-white border-amber-500">1</Button>
