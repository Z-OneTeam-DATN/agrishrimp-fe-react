...
export default function AdminTopHeader() {
  ...
  const formattedDate = mounted ? time.toLocaleDateString('vi-VN', { 
    weekday: 'short', 
    year: 'numeric', 
    month: '2-digit', 
    day: '2-digit' 
  ...
  const formattedTime = mounted ? time.toLocaleTimeString('vi-VN', { 
    hour: '2-digit', 
    minute: '2-digit',
    second: '2-digit'
  }) : "--:--:--";
    ...
    <header className="h-[64px] border-b border-slate-200 bg-white/80 backdrop-blur-md flex items-center justify-between px-6 sticky top-0 z-50 shadow-sm">
      <div className="flex items-center gap-6 flex-1">
        ...
        <div className="flex items-center gap-3 pr-6 border-r border-slate-100">
          ...
          <div className="flex flex-col">
            <span className="text-[13px] font-black text-slate-900 leading-none tracking-tight uppercase">AGRISHRIMP CO.</span>
            <span className="text-[10px] font-bold text-emerald-600 uppercase mt-0.5 tracking-wider">System Admin</span>
          </div>
        </div>
        ...
        <div className="hidden xl:flex items-center gap-4 text-slate-500 ml-2">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-100/50">
            ...
            <span className="text-[13px] font-bold text-slate-700 font-mono tracking-wider">{formattedTime}</span>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border border-emerald-100 rounded-full">
          ...
          <span className="text-[12px] font-bold text-emerald-700">Hệ thống tổng</span>
        ...
        <div className="flex items-center gap-1 border-l border-slate-100 pl-4">
          <Button variant="ghost" size="icon" className="relative text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-full transition-colors">
            <Bell size={20} />
            <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-rose-500 rounded-full border-2 border-white"></span>
          </Button>
          
          <Button variant="ghost" size="icon" className="text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-full transition-colors">
            <CircleHelp size={20} />
          </Button>
        </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <div className="flex items-center gap-3 cursor-pointer pl-2 pr-1 py-1 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all ml-2 group">
              <div className="text-right hidden sm:block">
                <p className="text-[13px] font-black text-slate-800 leading-none group-hover:text-emerald-600 transition-colors">Admin Agri</p>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mt-1">Quản trị viên</p>
              </div>
          ...
          <DropdownMenuContent align="end" className="w-64 mt-3 shadow-2xl border-slate-100 rounded-xl p-2">
            <DropdownMenuLabel className="px-3 py-3">
              <div className="flex flex-col">
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Quyền hạn cao nhất</span>
                <span className="text-[13px] font-bold text-slate-800 mt-0.5">admin@agrishrimp.com</span>
              </div>
            </DropdownMenuLabel>
