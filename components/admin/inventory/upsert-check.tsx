...
export default function InventoryUpsert({
  ...
  const handleComplete = async () => {
    ...
    const checkId =
      initialData?.id ||
      (await InventoryCheckApiService.getDetail(formData.code)).id;
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-black text-slate-800 uppercase flex items-center gap-2">
              ...
              {mode === "view" && (
                <Badge className="bg-emerald-50 text-emerald-600 border-emerald-100 text-[9px]">
                  VIEW ONLY
                </Badge>
              )}
              {mode === "edit" && (
                <Badge className="bg-amber-50 text-amber-600 border-amber-100 text-[9px]">
                  EDIT MODE
                </Badge>
              )}
            </h1>
        ...
        <div className="flex items-center gap-2">
            ...
            <>
              {hasPermission(P.CHECK_UPDATE) && (
                <Button
                  ...
                  onClick={() =>
                    router.push(
                      `/admin/inventory-checks/${formData.code}?edit=true`,
                    )
                  }
                >
              ...
              {hasPermission(P.CHECK_APPROVE) && (
                <Button
                  size="sm"
                  className="h-8 bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold uppercase px-4"
          ...
          {mode !== "view" && (
            <Button
              size="sm"
              className="h-8 bg-blue-600 text-white text-[11px] font-bold uppercase px-4"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="h-8 flex items-center px-3 rounded-md bg-slate-100 border border-slate-200 text-[12px] font-black text-slate-600">
                <Hash size={12} className="mr-2 text-slate-400" />{" "}
                {formData.code}
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => {
                  const emp = employees.find(
                    (e) => e.fullName === v || e.username === v,
                  );
                  const name = emp ? emp.fullName : v;
                  if (formData.checkedBy.includes(name)) return;
                  const newVal = formData.checkedBy
                    ? `${formData.checkedBy}, ${name}`
                    : name;
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {checkedByNames.map((n, i) => (
                    <Badge
                      key={i}
                      variant="secondary"
                      className="text-[9px] py-0 h-4 flex items-center gap-1"
                    >
                      {n}
                      {mode !== "view" && (
                        <X
          ...
          <div className="p-5 flex flex-col h-full">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <Badge className="bg-slate-100 text-slate-500 text-[10px] border-none font-bold uppercase ml-2">
                  {items.length}
                </Badge>
              </div>
                  ...
                  {searchResults.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 shadow-xl rounded-lg z-50 max-h-[300px] overflow-auto divide-y divide-slate-50">
                      {searchResults.map((p) => (
                        <div
                          key={p.id}
                          className="p-2.5 hover:bg-blue-50 cursor-pointer flex justify-between items-center group"
                          onClick={() => addItem(p)}
                        >
              ...
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="hover:bg-transparent border-b border-slate-100">
                    <TableHead className="w-[50px] text-[10px] font-black uppercase text-slate-400 text-center">
                      STT
                    </TableHead>
                    <TableHead className="w-[140px] text-[10px] font-black uppercase text-slate-400">
                      Mã biến thể
                    </TableHead>
                    <TableHead className="text-[10px] font-black uppercase text-slate-400">
                      Tên biến thể
                    </TableHead>
                    <TableHead className="w-[70px] text-[10px] font-black uppercase text-slate-400 text-center">
                      ĐVT
                    </TableHead>
                    <TableHead className="w-[110px] text-right text-[10px] font-black uppercase text-blue-600">
                      Số lượng tồn
                    </TableHead>
                    <TableHead className="w-[110px] text-right text-[10px] font-black uppercase text-emerald-600">
                      Kiểm kê
                    </TableHead>
                    <TableHead className="w-[110px] text-right text-[10px] font-black uppercase text-rose-600">
                      Hàng lỗi
                    </TableHead>
                    <TableHead className="w-[110px] text-right text-[10px] font-black uppercase text-slate-400">
                      Chênh lệch
                    </TableHead>
                    <TableHead className="w-[200px] text-center text-[10px] font-black uppercase text-slate-400">
                      Nguyên nhân
                    </TableHead>
                    {mode !== "view" && (
                ...
                <TableBody>
                  {items.length === 0 ? (
                    <TableRow>
                      <TableCell
                        colSpan={mode === "view" ? 9 : 10}
                        className="h-40 text-center text-slate-300"
                      >
                    ...
                    items.map((item, index) => {
                      ...
                      return (
                        <TableRow
                          key={index}
                          className="hover:bg-slate-50/30 border-b border-slate-50"
                        >
                          <TableCell className="text-center text-[11px] font-bold text-slate-400">
                            {index + 1}
                          </TableCell>
                          <TableCell className="text-[12px] font-mono font-bold text-slate-600 uppercase">
                            {item.sku}
                          </TableCell>
                          <TableCell className="text-[12px] font-black text-slate-700">
                            {item.name}
                          </TableCell>
                          <TableCell className="text-center text-[12px] font-bold text-slate-500">
                            {item.unit}
                          </TableCell>
                          <TableCell className="text-right text-[13px] font-black text-blue-700">
                            {item.systemQuantity}
                          </TableCell>
