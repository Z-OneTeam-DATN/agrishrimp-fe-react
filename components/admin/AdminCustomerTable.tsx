...
import { Eye, Phone, User, MapPin, Lock, Unlock, ChevronLeft, ChevronRight } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
      ...
      <Table className="table-custom border-collapse w-full">
        <TableHeader>
          <TableRow className="bg-[#f4f6f8] hover:bg-[#f4f6f8] border-b border-[#eee]">
            <TableHead className="w-[80px] font-bold text-[#1f1f1f] text-[11px] uppercase p-3 pl-5">ID</TableHead>
        ...
        <TableBody>
          {customers?.length > 0 ? (
            customers.map((cus) => (
              ...
              >
                <TableCell className="p-3 pl-5 text-[12px] font-bold text-slate-500">#{cus.userId}</TableCell>
                <TableCell className="p-3">
                  <div className="flex items-center gap-3">
                    
                    {/* 👇 SỬA LẠI CHỖ NÀY THÀNH cus.avatarUrl */}
