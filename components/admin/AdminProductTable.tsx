      ...
      <Table className="table-custom border-collapse">
        <TableHeader>
          <TableRow className="bg-[#f0f0f0] hover:bg-[#f0f0f0] border-b border-[#ccc]">
            ...
            <TableHead className="font-bold text-[#1f1f1f] text-[11px] uppercase p-2 pl-4">Tên sản phẩm & SKU</TableHead>
            <TableHead className="w-[120px] font-bold text-[#1f1f1f] text-[11px] uppercase p-2">Thương hiệu</TableHead>
            <TableHead className="w-[100px] text-center font-bold text-[#1f1f1f] text-[11px] uppercase p-2">Tồn kho</TableHead>
            <TableHead className="w-[100px] text-center font-bold text-[#1f1f1f] text-[11px] uppercase p-2">Có thể bán</TableHead>
                ...
                >
                  ...
                  <TableCell className="p-2 pl-4">
                    <div className="flex flex-col">
                      ...
                      <span className="text-slate-500 text-[10px] font-mono">#{p.sku}</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="p-2 text-center">
                    <span className="text-[12px] font-bold text-slate-700">{p.inventory.toLocaleString()}</span>
                    <p className="text-[9px] text-slate-400 uppercase font-bold">({p.variants.length} SKU)</p>
                  </TableCell>

                  <TableCell className="p-2 text-center">
                    <span className="text-[12px] font-bold text-emerald-600">{p.available.toLocaleString()}</span>
                    <p className="text-[9px] text-slate-400 uppercase font-bold">Sẵn có</p>
                  </TableCell>

                  <TableCell className="p-2 text-center">
                    <span className={cn(
                      "text-[10px] font-bold px-1.5 py-0.5 rounded border tracking-tight uppercase",
                      p.status === "Đang kinh doanh" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-slate-100 text-slate-400 border-slate-200"
                    )}>
                      {p.status}
                    </span>
                  </TableCell>
                    ...
                    <TableCell colSpan={8} className="p-0 border-b border-[#eee]">
                      <div className="pl-[40px] pr-4 py-3 space-y-4">
                        <div className="space-y-2">
                          ...
                          {p.variants.map((v) => {
                            const priceNum = parseInt(v.price.replace(/\D/g, '')) || 0;
                            const costNum = parseInt(v.costPrice.replace(/\D/g, '')) || 0;
                            const margin = priceNum > 0 ? Math.round(((priceNum - costNum) / priceNum) * 100) : 0;
                            const marginColor = margin > 30 ? "text-emerald-600" : margin < 10 ? "text-rose-600" : "text-amber-600";
                              ...
                              >
                                <div className="col-span-3 flex items-center gap-3">
                                  <div className="w-8 h-8 border border-slate-200 rounded-[2px] flex items-center justify-center bg-slate-50 overflow-hidden">
                                    {v.image ? (
                                      <img src={v.image} className="w-full h-full object-cover p-0.5" />
                                    ) : (
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    <p className="text-[9px] text-slate-400 font-bold">#{v.id}</p>
                                  </div>
                                </div>

                                <div className="col-span-2 text-center">
                                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter mb-0.5">Kho / Bán</p>
                                  <p className="text-[11px] font-bold text-slate-600">{v.inventory} | <span className="text-blue-600">{v.sold}</span></p>
                                </div>

                                <div className="col-span-2 text-right">
                                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter mb-0.5">Giá vốn</p>
                                  <p className="text-[11px] font-bold text-slate-400">{v.costPrice}</p>
                                </div>

                                <div className="col-span-2 text-right">
                                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter mb-0.5">Giá sỉ</p>
                                  <p className="text-[11px] font-bold text-orange-600">{v.wholesalePrice}</p>
                                </div>

                                <div className="col-span-3 text-right pr-2">
                                  <div className="flex items-center justify-end gap-2">
                                    <span className={cn("text-[9px] font-bold uppercase px-1 bg-slate-50 border border-slate-100 rounded-[2px]", marginColor)}>
                                      {margin}%
                                    </span>
                                    <div className="text-right">
                                      <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter mb-0.5">Giá lẻ niêm yết</p>
                                      <p className="text-[12px] font-bold text-[#1f1f1f]">{v.price}</p>
                                    </div>
                                  </div>
                                </div>
                              </div>
