...
import { Pencil, Trash2, Layers, ChevronLeft, ChevronRight, Hash } from "lucide-react";
import { 
  Table, 
  TableBody, 
  TableCell, 
      ...
      <Table className="table-custom border-collapse">
        <TableHeader>
          <TableRow className="bg-[#f0f0f0] hover:bg-[#f0f0f0] border-b border-[#ccc]">
            <TableHead className="w-[250px] font-bold text-[#1f1f1f] text-[11px] uppercase p-2 pl-4">Tên thuộc tính</TableHead>
            <TableHead className="w-[150px] font-bold text-[#1f1f1f] text-[11px] uppercase p-2">Mã định danh</TableHead>
          ...
          {attributes.map((attr) => (
            <TableRow key={attr.id} className="hover:bg-[#f0f8ff] border-b border-[#eee] transition-colors">
              ...
              <TableCell className="p-2">
                <code className="bg-slate-50 text-slate-500 text-[10px] font-mono border border-slate-100 px-1.5 py-0.5 rounded uppercase">
                  {attr.code}
                </code>
              </TableCell>
              <TableCell className="p-2">
              ...
              <TableCell className="p-2 text-center font-bold text-slate-600 text-[12px]">
                {attr.useCount}
              </TableCell>
              <TableCell className="p-2">
                <span className={cn(
