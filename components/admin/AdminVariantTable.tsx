...
export function AdminVariantTable({
  attributes,
  onDelete,
}: AdminVariantTableProps) {
      ...
      <Table className="table-custom border-collapse">
        <TableHeader>
          <TableRow className="bg-[#f0f0f0] hover:bg-[#f0f0f0] border-b border-[#ccc]">
            ...
            <TableHead className="w-[200px] font-bold text-[#1f1f1f] text-[11px] uppercase p-2 pl-4">
              Tên thuộc tính
            </TableHead>
        ...
        <TableBody>
          {attributes.length > 0 ? (
                ...
                <TableCell className="p-2">
                  <div className="flex flex-wrap gap-1">
                    {attr.values?.map((val: string, i: number) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 bg-white border border-slate-200 text-slate-500 text-[11px] font-medium rounded-[2px] shadow-sm"
                      >
                        {val}
                      </span>
                    ))}
                ...
                <TableCell className="p-2 text-right pr-4">
                  <div className="flex justify-end gap-1">
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-8 w-8 hover:bg-blue-50 border border-transparent hover:border-blue-100 transition-all"
                      onClick={() =>
                        router.push(`/admin/variants/add?id=${attr.id}`)
                      }
                    >
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-8 w-8 hover:bg-rose-50 border border-transparent hover:border-rose-100 transition-all"
            ...
            <TableRow>
              <TableCell
                colSpan={6}
                className="text-center py-10 text-slate-300 italic"
              >
                Không có dữ liệu
              </TableCell>
            </TableRow>
