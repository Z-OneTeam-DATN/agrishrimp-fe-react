  ...
  return (
    <>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-slate-50/50 hover:bg-slate-50/50 border-b border-slate-100">
              ...
              <TableHead className="font-bold text-slate-500 text-[11px] uppercase p-[10px]">Ngày điều chuyển</TableHead>
              <TableHead className="font-bold text-slate-500 text-[11px] uppercase p-[10px]">Lý do</TableHead>
            ...
            {transfers.length > 0 ? (
              transfers.map((item: any) => (
                <TableRow key={item.code} className="hover:bg-slate-50/80 border-b border-slate-50 last:border-0 transition-colors cursor-pointer">
                  ...
                  <TableCell className="p-[10px] text-slate-800 font-bold">{item.description}</TableCell>
                  <TableCell className="p-[10px] text-slate-500 font-medium">{item.sourceWarehouse}</TableCell>
                  <TableCell className="p-[10px]">
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border uppercase tracking-tighter ${
                      ...
                      item.status === 'Đang vận chuyển' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-orange-50 text-orange-600 border-orange-100'
                    }`}>
                      {item.status}
                    </span>
                  </TableCell>
              ...
              <TableRow>
                <TableCell colSpan={6} className="h-[300px] text-center">
                  <div className="flex flex-col items-center justify-center text-slate-300">
                     ...
                     <p className="font-black text-slate-400 uppercase text-[11px] tracking-widest">Không có dữ liệu</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
      ...
      <div className="flex items-center justify-between px-6 py-3 border-t border-slate-100 bg-slate-50/30">
        <p className="text-[11px] font-bold text-slate-400 uppercase">Hiển thị <span className="text-slate-900">{transfers.length}</span> bản ghi</p>
