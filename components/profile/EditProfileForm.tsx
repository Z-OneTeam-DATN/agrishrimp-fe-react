...
export default function EditProfileForm({
  ...
  const onSubmit = async (data: UserData) => {
      try {
        ...
        // Giá trị từ thẻ <input type="date"> mặc định đã là chuỗi "YYYY-MM-DD"
        if (data.birthday) {
          if (typeof data.birthday === 'string') {
            formattedDate = data.birthday; // Giữ nguyên chuỗi chuẩn
        ...
        // 5. BẮT LỖI TỪ BACKEND CHUYÊN NGHIỆP
        console.error("🚨 Lỗi cập nhật (Chi tiết từ Backend):", error.response?.data);

        // Tự động tìm message lỗi trong cấu trúc JSON của Spring Boot
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-20 lg:pb-0">
      <div className="flex flex-col items-center mb-6">
        ...
        >
          <Image
            ...
            alt="Avatar"
            fill
            className={cn("rounded-full object-cover border-4 border-white shadow-md transition-all", isUploadingImage && "opacity-50")}
          />
          ...
          <button type="button" className="absolute bottom-0 right-0 bg-white border p-1.5 rounded-full text-gray-600 group-hover:text-emerald-600 shadow-sm transition-colors">
            <Camera size={14} />
          </button>
      ...
      <div className="space-y-2">
        ...
        <input
          {...register("fullname")}
          className={cn("w-full px-4 h-12 bg-white border rounded-lg text-sm focus:ring-1 focus:ring-emerald-500", errors.fullname ? "border-red-500" : "border-gray-200")}
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <input {...register("email")} disabled className="w-full px-4 h-12 bg-gray-50 border border-gray-100 rounded-lg text-sm text-gray-400 cursor-not-allowed" />
      </div>
      ...
      <div className="space-y-2">
        ...
        <input
          {...register("phone")}
          className={cn("w-full px-4 h-12 bg-white border rounded-lg text-sm focus:ring-1 focus:ring-emerald-500", errors.phone ? "border-red-500" : "border-gray-200")}
        />
      </div>
          ...
          {[
            ...
            <label
              ...
              className={cn(
                ...
                currentGender === g.value ? "bg-emerald-50 border-emerald-500 text-emerald-700" : "bg-white border-gray-200 text-gray-500"
              )}
            >
      ...
      <div className="space-y-2">
        ...
        <input type="date" {...register("birthday")} className={cn("w-full px-4 h-12 bg-white border rounded-lg text-sm focus:ring-1 focus:ring-emerald-500", errors.birthday ? "border-red-500" : "border-gray-200")} />
      </div>
