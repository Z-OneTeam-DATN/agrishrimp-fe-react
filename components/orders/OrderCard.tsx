  ...
  return (
    <div className="bg-white rounded-lg mb-3 shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-200">
      ...
      <div className="flex justify-between items-center px-4 py-3 border-b border-gray-50 bg-gray-50/30">
        ...
        <div className={cn("uppercase text-xs font-bold flex items-center", statusConfig.className)}>
      ...
      <Link href={`/orders/${order.id}`} className="block px-4 py-3 cursor-pointer hover:bg-gray-50 transition-colors">
        ...
        {order.status === 'CANCELLED' && (
            <div className="bg-[#fff5f5] border-l-[3px] border-red-500 rounded p-2.5 mb-3">
                ...
                <div className="text-xs text-gray-500 mt-0.5">Lý do: Muốn thay đổi địa chỉ nhận hàng</div>
            </div>
        )}
        ...
        {order.items.map((item, index) => (
          <div key={index} className={cn("flex gap-3 py-2", { "border-b border-dashed border-gray-100": index < order.items.length - 1 })}>
            ...
            <div className={`text-right text-sm font-medium ${order.status === 'CANCELLED' ? 'text-gray-400' : 'text-gray-900'}`}>
        ...
        {order.status === 'SHIPPING' && (
            <div className="mt-2 pt-2 border-top border-gray-100 flex items-start gap-2">
                ...
                <div>
                    <div className="text-xs text-[#2d9f8d] font-medium">Đơn hàng đã đến kho Cần Thơ SOC</div>
