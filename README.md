...
- `Bác sĩ AI chẩn đoán bệnh tôm`: người dùng tải ảnh tôm bệnh và nhập thêm triệu chứng. Web gửi dữ liệu sang backend Spring Boot, backend gọi AI service FastAPI dùng `YOLO11n` để nhận diện 4 lớp `BG`, `Healthy`, `WSSV` và `Yellowhead` từ ảnh, sau đó lấy top dự đoán theo độ tin cậy. Backend dùng kết quả bệnh, triệu chứng người dùng nhập và danh sách sản phẩm còn khả dụng để gọi `Gemini 2.5 Flash` sinh phác đồ điều trị dạng có cấu trúc theo từng giai đoạn. Kết quả cuối cùng hiển thị ngay trên web gồm tên bệnh, độ tin cậy, mô tả dấu hiệu, nguyên nhân có thể, từng bước xử lý và các sản phẩm gợi ý có thể thêm thẳng vào giỏ hàng.
