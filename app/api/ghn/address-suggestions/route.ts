...
const sanitizeSuggestions = (raw: unknown, scope: Scope) => {
  ...
  return dedupeSuggestions(
    raw
      .map((item: any): AddressSuggestion | null => {
    ...
    raw
      .map((item): AddressSuggestion | null => {
        ...
        const street = [addr.house_number, addr.road, addr.building]
          ...
          .join(" ")
          .trim();
        const label = [street, ward, district, province]
          .filter(Boolean)
          .join(", ");
      ...
      .filter(
        (suggestion): suggestion is AddressSuggestion =>
          ...
          isSuggestionWithinScope(suggestion, scope),
      )
      .slice(0, 8),
  );
};
...
const fetchBackendSuggestions = async (input: string, scope: Scope) => {
  ...
  if (hasText(scope.province))
    backendUrl.searchParams.set("province", scope.province);
  if (hasText(scope.district))
    backendUrl.searchParams.set("district", scope.district);
