...
export default function CheckoutPage() {
  ...
  // Lấy vị trí user ngay khi load trang
  useUserLocation()
  ...
  // Bước 1 → 2: Nhận delivery info và gọi prepare order
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 md:pb-10">
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              {/* Error — không tìm được chi nhánh */}
              {prepareMutation.isError && !prepareMutation.isPending && (
                <div className="border border-red-200 bg-red-50 rounded-xl p-4 space-y-3">
                  ...
                  >
                    ← Thay đổi địa chỉ giao
                  </button>
                </div>
              )}

              {/* Success — canFulfill = false */}
              {!prepareMutation.isPending &&
                prepareOrderResponse &&
                !prepareOrderResponse.canFulfill && (
                  <OutOfStockWarning
                    items={prepareOrderResponse.outOfStockItems}
                    onOrderPartial={handleProceedToPayment}
                  />
                )}

              {/* Success — canFulfill = true */}
              {!prepareMutation.isPending && prepareOrderResponse?.canFulfill && (
            ...
            {!prepareMutation.isPending && (
              <div className="flex gap-3">
                <button
                  onClick={() => setStep(1)}
                  className="flex items-center gap-2 px-5 py-2.5 border border-gray-300 text-gray-700 text-sm font-medium rounded-xl hover:bg-gray-50 transition-colors"
                >
                ...
                {prepareOrderResponse?.canFulfill && (
                  <button
                    ...
                    className="flex items-center gap-2 flex-1 justify-center py-2.5 bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold rounded-xl transition-colors"
                  >
                    Chọn thanh toán <ArrowRight size={15} />
                  </button>
                )}
              </div>
            )}
        ...
        {step === 3 && prepareOrderResponse && (
          <div className="space-y-4">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              {/* ── Thông tin bổ sung theo từng phương thức ── */}
              ...
              {/* TRANSFER — hiện thông tin tài khoản */}
              ...
              {paymentMethod === "COD" && (
                <div className="mt-3 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-xs text-gray-600 flex items-start gap-2">
                  ...
                  <span>
                    ...
                    <strong>{prepareOrderResponse && formatMoney(prepareOrderResponse.totalAmount)}</strong>{" "}
                    khi nhận hàng.
                  </span>
                </div>
              )}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 flex-1 py-2.5 bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
      ...
      {/* ── MODAL: CONFLICT (409 — hàng hết sau khi prepare) ── */}
      {showConflictModal && (
      ...
      {/* ── MODAL: TOKEN EXPIRED (400 — prepareToken > 30 phút) ── */}
