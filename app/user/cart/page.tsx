  ...
  if (items.length === 0) {
    return (
      <div className="container mx-auto px-4 py-16 text-center min-h-[60vh] flex flex-col items-center justify-center">
        ...
        />
        <h5 className="text-xl font-bold text-gray-500 mb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-lg md:rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="divide-y divide-gray-100">
                {items.map((item) => (
                  <div
                    key={item.id}
                    className="relative flex p-3 md:p-4 hover:bg-gray-50 transition-colors gap-3 bg-white group"
                  >
                    ...
                    <div className="flex-grow flex flex-col justify-between w-[calc(100%-120px)] md:w-auto md:flex-row md:items-center">
                      ...
                      <div className="pr-6 md:pr-0 md:flex-grow md:w-auto">
                        <Link
                          href="#"
                          className="text-sm font-medium text-gray-800 hover:text-teal-600 line-clamp-2 mb-1"
                        >
                          {item.name}
                        </Link>
                        ...
                        <div className="text-[10px] md:text-xs text-orange-500 md:hidden">
                          Còn lại: {item.stock}
                        </div>
                      </div>
                        ...
                        <div className="flex items-center md:w-32 md:justify-center">
                          <div className="flex items-center border border-gray-300 rounded overflow-hidden h-8 bg-white shadow-sm">
                            <button
                              ...
                              className="w-8 h-full flex items-center justify-center hover:bg-gray-100 active:bg-gray-200 transition-colors text-gray-600"
                            >
                              <Minus size={14} />
                            </button>
                            <input
                              type="number"
                              className="w-10 h-full text-center text-sm font-semibold border-x border-gray-200 focus:outline-none"
                              value={item.quantity}
                              readOnly
                            />
                            <button
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg md:rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 md:sticky md:top-24">
              <h5 className="font-bold text-base md:text-lg text-gray-800 mb-4 md:mb-6">
                Chi tiết thanh toán
              </h5>
          ...
          <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
            <div className="flex gap-3 items-center">
              <div className="flex flex-col flex-1">
                ...
                <span className="text-lg font-extrabold text-teal-600 text-right">
                  {formatMoney(finalTotal)}
                </span>
              </div>
              <Link
                href="/user/checkout"
                className={`w-1/2 flex items-center justify-center font-bold py-3 rounded-lg shadow-sm active:scale-95 transition-transform text-white ${totalCount > 0 ? "bg-gradient-to-r from-teal-600 to-green-600" : "bg-gray-300"}`}
              >
                MUA HÀNG ({totalCount})
              </Link>
            </div>
