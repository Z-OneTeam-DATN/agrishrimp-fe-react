...
export default function CheckoutPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32 md:pb-10"> {/* Padding bottom lớn cho mobile để tránh bị che bởi thanh Sticky */}
      ...
      <div className="container mx-auto px-4 mt-4 md:mt-0">
        <form onSubmit={(e) => { e.preventDefault(); alert('Đặt hàng thành công!'); }} className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
               ...
               <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50 p-3 md:p-4 rounded-lg border border-gray-200">
                  <div className="w-full">
                     <div className="font-bold text-gray-800 mb-1 flex flex-wrap gap-2 items-center">
                        ...
                        <span className="text-teal-700 block sm:inline">{selectedAddress.phone}</span>
                     </div>
                    ...
                    >
                       <div className="flex items-center gap-3">
                          ...
                          <div>
                             ...
                             <div className="text-[11px] md:text-xs text-gray-500">{method.date}</div>
                          </div>
                       </div>
                       <div className="font-bold text-gray-900 text-sm md:text-base">{formatMoney(method.price)}</div>
                    </label>
                  ))}
               </div>
            </div>
            ...
            <div className="bg-white rounded-lg md:rounded-xl shadow-sm border border-gray-100 p-4 md:p-6">
               ...
               <div className="space-y-3">
                  {[
                    ...
                    { val: 'BANK', label: 'Chuyển khoản ngân hàng', sub: 'Chuyển khoản 24/7', icon: 'https://cdn-icons-png.flaticon.com/512/2534/2534204.png' }
                  ].map((pm) => (
          ...
          <div className="lg:col-span-1">
             <div className="bg-white rounded-lg md:rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 md:sticky md:top-24">
                <h5 className="font-bold text-base md:text-lg text-gray-800 mb-3 md:mb-4 pb-2 border-b border-gray-100 flex items-center gap-2">
                   <ShoppingBag size={18} className="md:hidden"/> Đơn hàng ({CART_ITEMS.length})
                </h5>
                ...
                <div className="hidden md:block">
                    ...
                    <button type="submit" className="w-full py-3.5 bg-gradient-to-r from-teal-600 to-green-600 text-white font-bold rounded-xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all">
                        ĐẶT HÀNG
                    </button>
                    <p className="text-center mt-4 text-[11px] text-gray-400">
                        Nhấn "Đặt hàng" đồng nghĩa với việc bạn đồng ý tuân theo <Link href="#" className="underline hover:text-teal-600">Điều khoản</Link>
                    </p>
                </div>
             </div>
          </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:p-4">
           ...
           <div className="bg-white w-full sm:w-auto sm:max-w-lg relative z-10 overflow-hidden shadow-2xl rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[90vh] animate-in slide-in-from-bottom duration-300 sm:zoom-in-95">
              ...
              <div className="p-4 border-b flex justify-between items-center bg-gray-50 shrink-0">
                 ...
                 <button onClick={() => setIsAddressModalOpen(false)} className="p-1"><X size={20} className="text-gray-500" /></button>
              </div>
                 ...
                 {!isAddingNewAddress ? (
                          ...
                          {ADDRESSES.map((addr) => (
                             ...
                             >
                                ...
                                <div className="flex items-center gap-2 mb-1 pr-6">
                                   ...
                                   {addr.isDefault && <span className="text-[9px] sm:text-[10px] bg-gray-100 text-teal-700 border border-teal-200 px-1.5 py-0.5 rounded whitespace-nowrap">Mặc định</span>}
                                </div>
                                <div className="text-xs sm:text-sm text-gray-500 mb-1">{addr.phone}</div>
                                <div className="text-xs sm:text-sm text-gray-600 line-clamp-2">{addr.address}</div>
                             </label>
                          ))}
                       </div>
                    ...
                    <div className="space-y-3 sm:space-y-4 pb-10 sm:pb-0">
                       <div className="grid grid-cols-2 gap-3">
                          <div>
                             <label className="text-xs font-bold text-gray-700 mb-1 block">Họ tên <span className="text-red-500">*</span></label>
                             <input type="text" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-teal-500" placeholder="Tên của bạn"/>
                          </div>
                          <div>
                             <label className="text-xs font-bold text-gray-700 mb-1 block">SĐT <span className="text-red-500">*</span></label>
                             <input type="tel" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-teal-500" placeholder="09xxx"/>
                          </div>
                       </div>
                       <div>
                          <label className="text-xs font-bold text-gray-700 mb-1 block">Địa chỉ cụ thể <span className="text-red-500">*</span></label>
                          <textarea rows={2} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-teal-500" placeholder="Số nhà, đường..."></textarea>
                       </div>
                       <div>
                          <label className="text-xs font-bold text-gray-700 mb-2 block">Loại địa chỉ:</label>
                          <div className="flex gap-4">
                             <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="type" className="accent-teal-600" defaultChecked/> <span className="text-sm">Nhà riêng</span></label>
                             <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="type" className="accent-teal-600"/> <span className="text-sm">Công ty</span></label>
                          </div>
                       </div>
                    </div>
                 )}
              ...
              <div className="p-4 border-t bg-gray-50 flex justify-end gap-3 shrink-0 pb-8 sm:pb-4">
                 {!isAddingNewAddress ? (
                    <button onClick={() => setIsAddressModalOpen(false)} className="w-full sm:w-auto px-6 py-2.5 text-sm font-bold text-white bg-teal-600 rounded-lg shadow-sm active:bg-teal-700">Xác nhận</button>
                 ) : (
                    <>
                       <button onClick={() => setIsAddingNewAddress(false)} className="flex-1 sm:flex-none px-4 py-2.5 text-sm font-bold text-gray-600 bg-white border border-gray-300 rounded-lg">Trở lại</button>
                       <button onClick={() => setIsAddingNewAddress(false)} className="flex-1 sm:flex-none px-4 py-2.5 text-sm font-bold text-white bg-teal-600 rounded-lg shadow-sm">Hoàn thành</button>
                    </>
                 )}
              </div>
           </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:p-4">
          ...
          <div className="bg-white w-full sm:w-full sm:max-w-md relative z-10 overflow-hidden shadow-2xl rounded-t-2xl sm:rounded-2xl animate-in slide-in-from-bottom duration-300 sm:zoom-in-95 flex flex-col max-h-[80vh]">
            ...
            <div className="p-4 border-b flex justify-between items-center bg-gray-50 shrink-0">
               ...
               <button onClick={() => setIsVoucherModalOpen(false)} className="p-1"><X size={20} className="text-gray-500"/></button>
            </div>
                    ...
                    <div
                      ...
                      onClick={() => {
                         if (subTotal >= (voucher.minOrder || 0)) {
                           ...
                           alert(`Đơn chưa đạt ${formatMoney(voucher.minOrder || 0)}`);
                         }
                      }}
