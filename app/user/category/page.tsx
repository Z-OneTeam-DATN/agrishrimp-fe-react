...
import { ChevronDown, Filter, LayoutGrid, List, X, SlidersHorizontal, ArrowUpDown } from 'lucide-react'; // Thêm icon X, SlidersHorizontal
...
const PRODUCTS = Array.from({ length: 20 }).map((_, i) => ({
  id: i + 1,
  name: i % 2 === 0 ? 'Vi sinh xử lý đáy ao APA MINER' : 'Thức ăn tôm thẻ Grow Best 40 đạm',
...
const VIEWED_PRODUCTS = [
    { id: 101, name: 'Men vi sinh', price: '150.000 ₫', image: 'https://apanano.com/wp-content/uploads/APA-MINER-POX_Shrimp.jpg', category: 'Xử lý nước', rating: 5, reviewCount: 2, sold: 10, tag: null },
    { id: 102, name: 'Khoáng tạt', price: '90.000 ₫', image: 'https://apanano.com/wp-content/uploads/APA-MINER-POX_Shrimp.jpg', category: 'Khoáng chất', rating: 5, reviewCount: 1, sold: 5, tag: null }
];
...
export default function CategoryPage() {
  ...
  const applyPriceFilter = () => {
    ...
    setPriceRange([min, max]); setMinInput(min); setMaxInput(max);
    ...
    <>
        ...
        <div className="mb-6">
            ...
            <div className="px-2 mb-4">
                <Slider
                range min={0} max={10000000} step={10000} value={priceRange} onChange={handleSliderChange}
                trackStyle={[{ backgroundColor: '#0d9488' }]}
                handleStyle={[{ borderColor: '#0d9488', backgroundColor: '#fff', opacity: 1 }, { borderColor: '#0d9488', backgroundColor: '#fff', opacity: 1 }]}
            ...
            <div className="flex items-center gap-2 mb-3">
                <div className="relative w-1/2">
                    ...
                    <input type="number" value={minInput} onChange={(e) => handleInputChange(e, 'min')} className="w-full pl-5 pr-1 py-1.5 text-xs border border-gray-200 rounded focus:border-teal-500 outline-none" placeholder="Từ" />
                </div>
                <span className="text-gray-400">-</span>
                <div className="relative w-1/2">
                    <span className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400 text-xs">₫</span>
                    <input type="number" value={maxInput} onChange={(e) => handleInputChange(e, 'max')} className="w-full pl-5 pr-1 py-1.5 text-xs border border-gray-200 rounded focus:border-teal-500 outline-none" placeholder="Đến" />
                </div>
            </div>
            <button onClick={applyPriceFilter} className="w-full py-2 bg-teal-600 text-white text-xs font-bold rounded hover:bg-teal-700 transition-colors uppercase">
                Áp dụng
            </button>
        </div>
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-10">
      ...
      {showMobileFilter && (
        <div className="fixed inset-0 bg-black/50 z-50 transition-opacity" onClick={() => setShowMobileFilter(false)}></div>
      )}
      <div className={`fixed inset-y-0 left-0 w-[85%] max-w-xs bg-white z-[51] shadow-2xl transform transition-transform duration-300 ease-in-out ${showMobileFilter ? 'translate-x-0' : '-translate-x-full'}`}>
         <div className="p-4 flex items-center justify-between border-b border-gray-100 bg-gray-50">
             ...
             <button onClick={() => setShowMobileFilter(false)} className="p-1 hover:bg-gray-200 rounded-full text-gray-500"><X size={20}/></button>
         </div>
      ...
      <div className="container mx-auto px-4 py-3 md:py-4">
        <nav className="text-xs md:text-sm text-gray-500 flex items-center gap-2 whitespace-nowrap overflow-hidden text-ellipsis">
           ...
           <span className="text-gray-400">/</span>
           <span className="font-bold text-gray-800 truncate">Sản phẩm nuôi tôm</span>
        </nav>
      </div>
          ...
          <main className="lg:col-span-3">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4">
                   <div className="flex items-center justify-between">
                       <h1 className="text-base md:text-xl font-bold text-gray-800 flex flex-col md:flex-row md:items-baseline gap-0.5 md:gap-2">
                          Chăm Sóc Ao Nuôi
                          <span className="text-xs md:text-sm font-normal text-gray-500">(2656 sản phẩm)</span>
                       </h1>
                   ...
                   <div className="flex items-center gap-2 md:gap-4 text-xs md:text-sm overflow-x-auto pb-1 md:pb-0 scrollbar-hide">
                       <span className="text-gray-500 whitespace-nowrap hidden md:inline">Sắp xếp:</span>
                       <button className="font-bold text-teal-600 whitespace-nowrap bg-teal-50 md:bg-transparent px-2 py-1 rounded md:px-0 md:py-0">Bán chạy</button>
                       <button className="text-gray-600 hover:text-teal-600 whitespace-nowrap px-2 py-1 md:px-0">Mới nhất</button>
                       <button className="text-gray-600 hover:text-teal-600 whitespace-nowrap flex items-center gap-1 px-2 py-1 md:px-0">Giá <ArrowUpDown size={12} className="md:hidden"/></button>
             ...
             <div className="flex justify-center md:justify-end mb-8">
                <nav className="flex gap-1">
                   <button className="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded border border-teal-600 bg-teal-600 text-white font-bold text-xs md:text-sm">1</button>
                   <button className="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded border border-gray-200 text-gray-600 hover:border-teal-600 hover:text-teal-600 bg-white transition-colors text-xs md:text-sm">2</button>
                   <span className="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center text-gray-400 text-xs md:text-sm">...</span>
                   <button className="px-2 md:px-3 h-8 md:h-9 flex items-center justify-center rounded border border-gray-200 text-gray-600 hover:border-teal-600 hover:text-teal-600 bg-white transition-colors text-xs md:text-sm">Cuối</button>
                </nav>
             </div>
             ...
             <div className="bg-white p-3 md:p-6 rounded-lg md:rounded-xl shadow-sm border border-gray-100 mb-6">
                ...
                <div className="flex md:grid md:grid-cols-5 gap-2 md:gap-4 overflow-x-auto pb-2 md:pb-0 snap-x md:snap-none no-scrollbar">
                   {PRODUCTS.slice(0, 5).map((prod) => (
                      <div key={prod.id + 99} className="min-w-[150px] md:min-w-0 snap-start">
                         <ProductCard {...prod} />
                      </div>
                   ))}
                </div>
             ...
             <div className="bg-white p-3 md:p-6 rounded-lg md:rounded-xl shadow-sm border border-gray-100">
                ...
                <div className="flex gap-2 md:gap-4 overflow-x-auto pb-2 scrollbar-hide">
                   {VIEWED_PRODUCTS.map((prod) => (
                      <div key={prod.id} className="min-w-[140px] w-[140px] md:min-w-[180px] md:w-[180px]">
