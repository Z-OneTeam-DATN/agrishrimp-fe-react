        ...
        {stage.products?.length > 0 ? (
          <div>
            ...
            <div className="mt-4 rounded-lg border border-dashed border-[#376E60]/25 bg-[#f4faf7] px-3 py-2 text-right text-sm font-semibold text-slate-700">
              Tạm tính giai đoạn: <span className="text-[#376E60]">{formatPrice(subtotal)}</span>
            </div>
          </div>
        ) : (
          <div className="rounded-xl bg-slate-50 px-4 py-6 text-center text-sm italic text-slate-400">
            Giai đoạn này chưa có sản phẩm gợi ý.
          </div>
        )}
      </div>
    </div>
...
export default function TreatmentResultPage() {
  ...
  const resolveVariantId = async (productId: number) => {
    ...
    const product = await PublicProductService.getById(productId);
    const preferredVariant =
      product.variants.find((variant) => variant.status === "ACTIVE" && variant.quantity > 0) ||
      product.variants.find((variant) => variant.status === "ACTIVE") ||
      product.variants[0];

    if (!preferredVariant?.id) {
      throw new Error(`Không tìm thấy biến thể hợp lệ cho sản phẩm ${productId}`);
    }

    variantIdCache.current.set(productId, preferredVariant.id);
  ...
  const addProductsToCart = async (products: AiDoctorSuggestedProduct[], key: string, label: string) => {
    ...
    try {
      ...
      if (status === 401 || status === 403) {
        ...
        toast.error("Không thể thêm đủ sản phẩm vào giỏ. Vui lòng thử lại.");
      }
    } finally {
      setAddingKey(null);
    }
  ...
  if (!diagnosisId) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#f5f7f9] px-4 text-center">
        ...
        <h1 className="mb-2 text-xl font-bold text-slate-900">Chưa có dữ liệu chẩn đoán</h1>
        <p className="mb-6 text-sm text-slate-500">
          Hãy bắt đầu một cuộc trò chuyện với bác sĩ AI để tạo báo cáo.
        </p>
        <Link
          href="/ai-doctor"
      ...
      <div className="w-full px-4 lg:px-6 2xl:px-10">
        <div className="mb-6 flex flex-col gap-4 border-b border-gray-200 pb-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs font-mono text-slate-500">
              <span className="rounded bg-slate-200 px-2 py-0.5">ID: #{diagnosisId}</span>
              <span>•</span>
              <span>Ngày lập: {formatDateTime(diagnosis?.createdAt)}</span>
            </div>
          </div>
        ...
        {diagnosisQuery.isLoading && !diagnosis ? (
          <div className="flex min-h-[420px] flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white">
            ...
            <p className="text-sm text-slate-500">Đang tải kết quả chẩn đoán...</p>
          </div>
        ) : diagnosisQuery.isError && !diagnosis ? (
          <div className="rounded-2xl border border-red-100 bg-red-50 px-5 py-10 text-center">
            <p className="text-lg font-bold text-red-600">Không tải được báo cáo chẩn đoán.</p>
                  ...
                  <div className="p-4">
                    ...
                    <div className="mb-4 rounded-md border border-slate-100 bg-slate-50 p-3">
                      ...
                      <p className="text-sm leading-relaxed text-slate-700">
                        {diagnosis.signsSummary || "AI chưa trả mô tả tổng quan."}
                      </p>
                    </div>
                      ...
                      {secondaryPredictions.map((prediction) => (
                        ...
                        >
                          ...
                          <span className="text-xs font-bold text-slate-400">
                            {Number(prediction.confidencePercent || 0).toFixed(1)}%
                          </span>
                        </div>
                      ))}
                    </div>
                ...
                <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {stageTotals.map((total, index) => (
                      ...
                      >
                        <span className="font-medium text-slate-600">Giai đoạn {index + 1}</span>
            ...
            <div className="space-y-6 lg:col-span-8">
              {diagnosis.treatmentStages?.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 text-gray-400">
                  ...
                  <p className="text-sm font-medium">Không có dữ liệu phác đồ điều trị.</p>
                </div>
              )}
            </div>
          </div>
