...
import { ChevronDown, Filter, LayoutGrid, List, ChevronRight } from 'lucide-react';
                ...
                <div className="mb-6">
                   ...
                   <ul className="space-y-2">
                      {CATEGORIES.map((cat, idx) => (
                        <li key={idx}>
                           <Link href="#" className={`text-sm flex items-center justify-between group ${cat.active ? 'text-[#329965] font-bold' : 'text-gray-600 hover:text-[#329965]'}`}>
                              <span className="flex items-center gap-2">
                                 <span className={`w-1.5 h-1.5 rounded-full ${cat.active ? 'bg-[#329965]' : 'bg-gray-300 group-hover:bg-[#329965]'}`}></span>
                                 {cat.name}
                              </span>
                ...
                <div className="mb-6">
                   ...
                   <div className="px-2 mb-4">
                      <Slider
                        ...
                        handleStyle={[
                            { borderColor: '#329965', backgroundColor: '#fff', opacity: 1 },
                            { borderColor: '#329965', backgroundColor: '#fff', opacity: 1 }
                        ]}
                   ...
                   <div className="space-y-2 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
                      {BRANDS.map((brand) => (
                         <label key={brand.id} className="flex items-center gap-3 cursor-pointer group">
                            <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-[#329965] focus:ring-[#329965] cursor-pointer" />
                            <span className="text-sm text-gray-600 group-hover:text-[#329965] transition-colors flex-1">{brand.name}</span>
          ...
          <main className="lg:col-span-9">
             <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4 text-sm overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                   ...
                   <button className="font-bold text-[#329965] whitespace-nowrap">Bán chạy</button>
                   <button className="text-gray-600 hover:text-[#329965] whitespace-nowrap">Mới nhất</button>
                   <button className="text-gray-600 hover:text-[#329965] whitespace-nowrap">Giá thấp ➜ cao</button>
                </div>
             </div>
             ...
             <div className="flex justify-center md:justify-end">
                <nav className="flex gap-1">
                   <button className="w-9 h-9 flex items-center justify-center rounded border border-[#329965] bg-[#329965] text-white font-bold text-sm">1</button>
                   <button className="w-9 h-9 flex items-center justify-center rounded border border-gray-200 text-gray-600 hover:border-[#329965] hover:text-[#329965] bg-white transition-colors text-sm">2</button>
                   <button className="w-9 h-9 flex items-center justify-center rounded border border-gray-200 text-gray-600 hover:border-[#329965] hover:text-[#329965] bg-white transition-colors text-sm">3</button>
                   <span className="w-9 h-9 flex items-center justify-center text-gray-400">...</span>
                   <button className="px-3 h-9 flex items-center justify-center rounded border border-gray-200 text-gray-600 hover:border-[#329965] hover:text-[#329965] bg-white transition-colors text-sm">Cuối</button>
                </nav>
             </div>
          </main>
        </div>
