...
const initialPonds = [
  ...
  },
  {
    ...
    species: "(Chưa thả)",
    status: "INACTIVE",
    description: "Ao đang phơi đáy, chờ vụ mới.",
  },
];
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-6 space-y-4">
        {ponds.map((pond) => (
          <div
            key={pond.id}
            className="border border-gray-100 rounded-lg p-4 hover:shadow-md transition-all duration-200 bg-white"
          >
            <div className="flex flex-col sm:flex-row gap-4">
              <div
                className={`w-16 h-16 rounded-lg flex items-center justify-center shrink-0 ${pond.status === "ACTIVE" ? "bg-blue-50 text-blue-500" : "bg-gray-100 text-gray-400"}`}
              >
                {pond.status === "ACTIVE" ? (
                  <Droplets size={32} />
                ) : (
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-3 text-xs text-gray-600 mb-2">
                  <div className="flex items-center gap-1 bg-gray-50 px-2 py-1 rounded">
                    <Maximize2 size={14} /> {pond.area.toLocaleString()} m²
                  </div>
                  <div className="flex items-center gap-1 bg-gray-50 px-2 py-1 rounded">
                    <Ruler size={14} /> Sâu {pond.depth}m
                  </div>
                  <div className="flex items-center gap-1 bg-gray-50 px-2 py-1 rounded">
                    <Fish size={14} /> {pond.species}
                  </div>
                </div>
                <div className="flex gap-3 pt-3 border-t border-dashed border-gray-100">
                  ...
                  <Link
                    href={`/ponds/${pond.id}`}
                    className="text-xs font-bold text-gray-600 hover:text-[#2d9f8d] flex items-center gap-1"
                  >
                    <Edit2 size={14} /> Sửa
                  </Link>
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-gray-600 hover:text-red-600 flex items-center gap-1"
                  >
                    <Trash2 size={14} /> Xóa
                  </button>
