...
export default function ReturnListPage() {
  ...
  const returnOrders = [
    ...
    },
    {
      ...
      shopResponse: 'Hình ảnh bằng chứng không rõ ràng, bao bì sản phẩm đã bị xé rách không còn nguyên vẹn.'
    }
  ];
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'PROCESSING':
        return <span className="flex items-center gap-1 px-3 py-1 rounded-full bg-[#fff8e1] text-[#f5a623] border border-[#ffe082] text-[10px] font-bold uppercase"><Clock size={12}/> Đang xử lý</span>;
      case 'COMPLETED':
        return <span className="flex items-center gap-1 px-3 py-1 rounded-full bg-[#e8f5e9] text-[#2e7d32] border border-[#c8e6c9] text-[10px] font-bold uppercase"><CheckCircle2 size={12}/> Đã hoàn tiền</span>;
      case 'REJECTED':
        return <span className="flex items-center gap-1 px-3 py-1 rounded-full bg-[#ffebee] text-[#c62828] border border-[#ffcdd2] text-[10px] font-bold uppercase"><XCircle size={12}/> Bị từ chối</span>;
      default: return null;
    }
  };
