...
import { RegisterRequest, UserResponse,AuthResponse } from "@/app/types/auth.schema";
...
export class AuthService {
  ...
  static async loginNext(request: LoginFormValues): Promise<AuthResponse> {
    const response = await apiNext.post<AuthResponse>(`${this.PREFIX}/login`, request)
    return response.data
  }

  static async logout(): Promise<{ res: { message: string } }> {
    const response = await apiJava.post<{ res: { message: string } }>(`${this.PREFIX}/logout`)
    return response.data
  }
  ...
  static async refresh(refreshToken: string): Promise<AuthResponse> {
    const response = await apiJava.post<AuthResponse>(`${this.PREFIX}/refresh`, { refreshToken })
    return response.data
  }
