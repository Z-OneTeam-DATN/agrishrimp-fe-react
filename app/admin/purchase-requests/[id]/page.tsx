...
import {
  Loader2, ChevronLeft, ShoppingCart, CheckCircle2, Clock,
  Truck, PackageCheck, AlertTriangle, XCircle, Plus,
  RefreshCcw, Package, FileText, CalendarDays,
} from "lucide-react";
...
export default function PurchaseRequestDetailPage() {
  ...
  const handleCreateReceipt = () => {
    router.push(`/admin/receipts/new?purchaseRequestId=${pr?.id}&supplierCode=${pr?.supplierCode}&branchName=${encodeURIComponent(pr?.branchName ?? "")}`);
  };
    ...
    <div className="max-w-6xl mx-auto py-6 px-4 space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {pr.status === "DRAFT" && (
            <>
              <Link href={`/admin/purchase-requests/${pr.id}/edit`}>
                <Button variant="outline" size="sm" className="h-8 text-[12px] font-bold">Sửa</Button>
              </Link>
              <Button
                size="sm"
                className="h-8 bg-amber-500 hover:bg-amber-600 text-white text-[12px] font-bold rounded-[3px]"
                onClick={() => setConfirmAction({ type: "submit", label: "Gửi duyệt" })}
              >
                Gửi duyệt
              </Button>
            </>
          ...
          {pr.status === "PENDING_APPROVAL" && (
            <>
              <Button variant="outline" size="sm" className="h-8 text-[12px] font-bold text-red-600 border-red-200"
                onClick={() => setConfirmAction({ type: "reject", label: "Từ chối (trả về Nháp)" })}>
                Từ chối
              </Button>
              <Button
                size="sm"
                className="h-8 bg-blue-600 hover:bg-blue-700 text-white text-[12px] font-bold rounded-[3px]"
                onClick={() => setConfirmAction({ type: "approve", label: "Duyệt phiếu" })}
              >
                Duyệt
              </Button>
            </>
          ...
          {pr.status === "APPROVED" && (
            <Button
              ...
              onClick={() => setConfirmAction({ type: "sendToSupplier", label: "Gửi cho nhà cung cấp" })}
            >
          ...
          {["SENT_TO_SUPPLIER", "PARTIALLY_RECEIVED"].includes(pr.status) && (
            <Button variant="outline" size="sm" className="h-8 text-[12px] font-bold text-gray-600"
              onClick={() => setConfirmAction({ type: "close", label: "Đóng phiếu (force close)" })}>
              Đóng phiếu
            </Button>
          )}

          {["DRAFT", "PENDING_APPROVAL", "APPROVED"].includes(pr.status) && (
            <Button variant="outline" size="sm" className="h-8 text-[12px] font-bold text-red-500 border-red-200"
              onClick={() => setConfirmAction({ type: "cancel", label: "Hủy phiếu yêu cầu" })}>
              Hủy
            </Button>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={card.label} className="bg-white border border-slate-200 rounded-sm p-4 shadow-sm">
            ...
            <div className="text-[13px] font-bold text-slate-700 truncate">{card.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-sm shadow-sm">
        <div className="px-5 py-3 border-b flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-400">{pr.items?.length ?? 0} mặt hàng</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-[12px]">
            <thead>
              <tr className="bg-slate-50 border-b">
                <th className="px-4 py-2.5 text-left font-black text-slate-500 uppercase tracking-wider">Sản phẩm</th>
                <th className="px-4 py-2.5 text-center font-black text-slate-500 uppercase tracking-wider">Yêu cầu</th>
                <th className="px-4 py-2.5 text-center font-black text-slate-500 uppercase tracking-wider">NCC giao</th>
                <th className="px-4 py-2.5 text-center font-black text-slate-500 uppercase tracking-wider text-green-700">Đạt QC</th>
                <th className="px-4 py-2.5 text-center font-black text-slate-500 uppercase tracking-wider text-red-600">Lỗi</th>
                <th className="px-4 py-2.5 text-center font-black text-slate-500 uppercase tracking-wider text-orange-600">Còn thiếu</th>
                <th className="px-4 py-2.5 text-right font-black text-slate-500 uppercase tracking-wider">Đơn giá</th>
                <th className="px-4 py-2.5 text-right font-black text-slate-500 uppercase tracking-wider">Thành tiền</th>
              </tr>
            </thead>
            <tbody>
              {(pr.items ?? []).map((item, idx) => {
                const fulfilled = (item.acceptedQty ?? 0) >= (item.requestedQty ?? 0);
                return (
                  <tr key={item.id ?? idx} className={cn(
                    ...
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        {item.imageUrl && (
                          <img src={item.imageUrl} alt="" className="w-7 h-7 rounded object-cover" />
                        )}
                        <div>
                          <div className="font-medium text-slate-700">{item.productName}</div>
                          <div className="text-[10px] text-slate-400">{item.productCode}</div>
                        </div>
                        {fulfilled && <CheckCircle2 size={13} className="text-green-500 ml-1" />}
                      </div>
                    </td>
                    <td className="px-4 py-2.5 text-center font-semibold">{item.requestedQty}</td>
                    <td className="px-4 py-2.5 text-center text-slate-600">{item.deliveredQty}</td>
                    <td className="px-4 py-2.5 text-center text-green-700 font-bold">{item.acceptedQty}</td>
                    <td className="px-4 py-2.5 text-center text-red-600 font-semibold">{item.defectiveQty}</td>
                    ...
                    <td className="px-4 py-2.5 text-right font-semibold text-slate-700">
                      {fmtCurrency((item.requestedQty ?? 0) * (item.unitPrice ?? 0))}
                    </td>
                  </tr>
                );
              })}
      ...
      <div className="bg-white border border-slate-200 rounded-sm shadow-sm">
        <div className="px-5 py-3 border-b flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-[11px] text-slate-500">
            <span>{pr.completedReceiptCount}/{pr.totalReceiptCount} đợt hoàn tất</span>
            {canCreateReceipt && hasRemaining && (
              <Button
                size="sm"
                variant="outline"
            ...
            <div className="relative">
              ...
              <div className="space-y-4">
                {pr.goodsReceipts.map((receipt, idx) => {
                  const statusInfo = RECEIPT_STATUS_MAP[receipt.status] ?? { label: receipt.status, cls: "bg-gray-100 text-gray-600" };
                  return (
                    <div key={receipt.id} className="relative flex gap-4 pl-2">
                      ...
                      <div className={cn(
                        ...
                        receipt.status === "PENDING"   ? "bg-amber-400" :
                        receipt.status === "APPROVED"  ? "bg-blue-500" :
                        receipt.status === "CANCELLED" ? "bg-gray-400" : "bg-red-400"
                      )}>
                        {idx + 1}
                      </div>
                      ...
                      <div className="flex-1 bg-slate-50 border border-slate-200 rounded-[4px] p-3 hover:border-blue-200 transition-colors">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] text-slate-400">{fmtDateTime(receipt.createdAt)}</span>
                        </div>

                        <div className="grid grid-cols-3 gap-3">
                          <div className="text-center">
                            <div className="text-[10px] text-slate-400 mb-0.5">NCC giao</div>
                            <div className="font-bold text-slate-700">{receipt.totalDelivered}</div>
                          </div>
                          <div className="text-center">
                            <div className="text-[10px] text-slate-400 mb-0.5">Đạt QC</div>
                            <div className="font-bold text-green-700">{receipt.totalAccepted}</div>
                          </div>
                          <div className="text-center">
                            <div className="text-[10px] text-slate-400 mb-0.5">Lỗi</div>
        ...
        <AlertDialogContent className="bg-white rounded-[6px] border border-slate-200 shadow-xl max-w-[400px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-600 text-[13px]">
              Bạn có chắc chắn muốn thực hiện thao tác này trên phiếu <strong>{pr.code}</strong>?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="h-8 text-[12px] font-bold" disabled={actionLoading}>
              Hủy bỏ
            </AlertDialogCancel>
            <AlertDialogAction
            ...
            >
              {actionLoading ? <Loader2 size={13} className="animate-spin mr-1.5" /> : null}
              Xác nhận
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
