...
import { getAttributes, deleteAttribute } from "@/app/services/AttributeService";
    ...
    <div className="space-y-3">
      ...
      <div className="bg-white border border-[#dcdcdc] rounded-[4px] shadow-sm overflow-hidden mb-8">
        ...
        {isLoading ? (
          ...
          <AdminVariantTable attributes={attributes} onDelete={(id) => setDeleteId(id)} />
        )}
      </div>
        ...
        <AlertDialogContent className="max-w-[400px] rounded-[8px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-center text-[13px] text-slate-500">
                Hành động này không thể hoàn tác. Bạn có chắc chắn muốn xóa thuộc tính này khỏi hệ thống không?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-row gap-2 mt-4">
            ...
            <AlertDialogAction
                ...
                disabled={isDeleting}
            >
              {isDeleting ? <Loader2 className="animate-spin" size={16} /> : "XÓA NGAY"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
