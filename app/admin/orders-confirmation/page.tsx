...
export default function ConfirmationPage() {
  ...
  const fetchOrders = useCallback(async (q?: string) => {
    setIsLoading(true);
    try {
      const data = await orderService.getBranchOrders("PENDING", q || undefined);
      setOrders(data);
    } catch {
  ...
  const handleConfirm = async (e: React.MouseEvent, orderId: number, orderCode: string) => {
    e.stopPropagation();
    try {
      await orderService.updateBranchOrderStatus(orderId, "CONFIRMED");
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={7} className="h-32 text-center text-slate-500">Không có đơn hàng nào chờ xác nhận.</TableCell></TableRow>
              ) : (
                orders.map((order) => {
                            ...
                            <div className="flex flex-col md:flex-row p-4 gap-6">
                              ...
                              <div className="flex-1">
                                <div className="flex justify-between items-center mb-3">
                                  <h3 className="text-[12px] font-bold text-slate-800">Danh sách sản phẩm</h3>
                                  <Button
                                    size="sm"
                                    className="h-[28px] bg-emerald-600 hover:bg-emerald-700 text-white text-[12px] font-bold shadow-sm"
                                    onClick={(e) => handleConfirm(e, order.orderId, order.orderCode)}
                                  >
