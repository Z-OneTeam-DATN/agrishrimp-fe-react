...
const StatusBadge = ({ status }: { status: OrderStatus | string }) => {
  ...
  return (
    <span className={cn("px-2.5 py-0.5 rounded-[10px] text-[11px] border whitespace-nowrap", mapped.styles)}>
      ○ {mapped.label}
    </span>
  );
};
    ...
    case "SHIPPING":
      return (
        <Button size="sm" className="h-[28px] bg-emerald-600 hover:bg-emerald-700 text-white text-[12px] font-bold"
          ...
          <CheckCircle size={13} className="mr-1.5" /> Hoàn thành
        </Button>
      );
    default:
      return null;
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      if (activeTab !== "all") {
        ...
        setOrders(prev =>
          prev.map(o => o.orderId === orderId ? { ...o, subOrderStatus: newStatus as OrderStatus } : o)
        );
        // Invalidate detail cache để reload nếu mở lại
        setDetailCache(prev => { const c = { ...prev }; delete c[orderId]; return c; });
      }
    } catch {
    ...
    <div className="p-4 space-y-4 bg-slate-50 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-[18px] font-bold text-slate-800">
          Danh sách đơn hàng{" "}
          <span className="font-normal text-slate-500 text-[15px]">({orders.length} đơn)</span>
        </h1>
        <Button
          variant="outline"
          size="sm"
          ...
          <Table>
            <TableHeader className="bg-[#f4f6f8]">
              <TableRow className="border-b border-slate-200 h-10">
                ...
                <TableHead className="text-[12px] font-bold text-slate-800 min-w-[130px]">Mã đơn hàng</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={8} className="h-32 text-center text-slate-400 animate-pulse">
                ...
                orders.map(order => {
                  ...
                  return (
                      ...
                      >
                        ...
                        <TableCell>
                          <span className="text-[13px] font-medium text-blue-600">{order.orderCode}</span>
                        </TableCell>
                            ...
                            <div className="flex flex-col md:flex-row p-4 gap-6">
                              ...
                              <div className="w-full md:w-[22%] space-y-3 border-r border-slate-200 pr-4">
                                ...
                                {order.estimatedDays && (
                                  <div>
                                    ...
                                    <p className="text-[12px] text-slate-600">
                                      {(() => {
                                        try { return new Date(order.estimatedDays!).toLocaleDateString("vi-VN"); }
                                        catch { return order.estimatedDays; }
                                      })()}
                                    </p>
                                  </div>
                                )}
                                ...
                                <div>
                                  ...
                                  <p className="text-[12px] font-medium text-slate-800">{formatCurrency(order.shippingFee)}</p>
                                </div>
                              </div>
                              ...
                              <div className="flex-1">
                                <div className="flex justify-between items-center mb-3">
                                  <h3 className="text-[12px] font-bold text-slate-800">Sản phẩm cần chuẩn bị</h3>
                                  <ActionButton
                                ...
                                <div className="border border-slate-200 rounded-[3px] overflow-hidden bg-white">
                                  <table className="w-full text-left">
                                    ...
                                    <tbody>
                                      {isLoadingDetail ? (
                                        <tr>
                                          <td colSpan={4} className="p-4 text-center text-[12px] text-slate-400">
                                            Đang tải sản phẩm...
                                          </td>
                                        </tr>
                                      ) : detail?.items?.length ? (
