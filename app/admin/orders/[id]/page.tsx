    ...
    <div className="bg-[#f0f2f5] min-h-screen pb-10 font-sans text-slate-800">
      ...
      <div className="max-w-[1200px] mx-auto px-4">
        ...
        <div className="grid grid-cols-12 gap-4">
            ...
            <div className="col-span-12 lg:col-span-8 space-y-4">
                ...
                <div className="bg-white rounded-sm shadow-sm border border-slate-200 overflow-hidden">
                    <div className="px-4 py-3 border-b border-slate-200 flex items-center gap-2 bg-slate-50/50">
                        <div className={cn("w-5 h-5 rounded-full border flex items-center justify-center p-[2px]", paymentStatus === 'paid' ? "border-emerald-500" : "border-orange-400")}>
                            <div className={cn("w-full h-full rounded-full", paymentStatus === 'paid' ? "bg-emerald-500" : "bg-orange-400")}></div>
                        </div>
                        <span className="font-bold text-[14px] text-slate-800">{paymentStatus === 'paid' ? "Đã thanh toán" : "Chưa thanh toán"}</span>
                    </div>
                    <div className="p-4">
                        <div className="flex justify-between items-center text-[13px] mb-2">
                            <span className="text-slate-600">Tổng tiền hàng</span>
                            <span className="font-bold text-slate-800">{order.pricing.subtotal.toLocaleString()}đ</span>
                        </div>
                        <div className="flex justify-between items-center text-[13px] font-bold pt-3 border-t border-slate-100 mt-2 text-[14px]">
                            <span className="text-slate-800">Thành tiền</span>
                            <span className="text-slate-800">{order.pricing.total.toLocaleString()}đ</span>
                        </div>
                        {paymentStatus === 'paid' && (
                            <div className="flex justify-between items-center text-[13px] mt-2 pt-2 border-t border-slate-100 text-emerald-600 font-medium">
                                <span>Đã thanh toán</span>
                                <span>{order.pricing.total.toLocaleString()}đ</span>
                            </div>
                        )}
                    </div>
                    {paymentStatus === 'unpaid' && (
                        <div className="px-4 py-3 border-t border-slate-100 flex justify-end gap-2 bg-white">
                            <Button variant="outline" className="bg-white border-blue-600 text-blue-600 hover:bg-blue-50 h-8 text-[13px] font-medium shadow-sm" onClick={() => setShowQRModal(true)}>Lấy mã QR</Button>
                            <Button className="bg-blue-600 hover:bg-blue-700 text-white h-8 text-[13px] font-medium shadow-sm" onClick={() => setShowReceiveMoneyModal(true)}>Nhận tiền</Button>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white rounded-sm shadow-sm border border-slate-200 p-4 shadow-sm">
                    <h3 className="font-bold text-[14px] mb-4 text-slate-700">Lịch sử đơn hàng</h3>
                    <div className="space-y-6 relative pl-2">
                        <div className="absolute top-2 left-[5px] bottom-2 w-[2px] bg-slate-100"></div>
                        {orderHistory.map((log, index) => (
                            <div key={index} className="flex gap-4 relative z-10 animate-in slide-in-from-left-2 duration-300">
                                <div className={cn("w-3 h-3 rounded-full mt-1.5 shrink-0 outline outline-4 outline-white transition-colors duration-500", log.action.includes("Đã xác nhận") ? "bg-emerald-500" : "bg-blue-500")}></div>
                                <div className="flex-1">
                                    <div className="flex gap-2 mb-0.5 items-baseline">
                                        <span className="font-bold text-[13px] text-slate-800">{log.time}</span>
                                        <span className="text-[13px] text-slate-800 font-medium">{log.actor}</span>
                                    </div>
                                    <p className={cn("text-[13px]", log.action.includes("Đã xác nhận") ? "text-emerald-600 font-bold" : "text-slate-600")}>{log.action}</p>
                                    {log.detail && (
                                        <div className="mt-1 flex items-center justify-between bg-slate-50 p-2 rounded border border-slate-100">
                                            <span className="text-[12px] text-slate-500 font-medium">{log.detail}</span>
                                            <ChevronDown size={14} className="text-slate-400 cursor-pointer"/>
                                        </div>
                                    )}
                                </div>
            ...
            <div className="col-span-12 lg:col-span-4 space-y-4">
                <div className="bg-white rounded-sm shadow-sm border border-slate-200 p-4 shadow-sm">
                    <h3 className="font-bold text-[13px] mb-2 text-slate-700">Nguồn đơn</h3>
                    <div className="flex items-center gap-2">
                        <div className="w-5 h-5 bg-amber-400 rounded flex items-center justify-center text-white shadow-sm"><Store size={12}/></div>
                        <span className="text-[13px] font-medium text-slate-800">{order.info.source}</span>
                    </div>
                </div>
                <div className="bg-white rounded-sm shadow-sm border border-slate-200 p-4 space-y-4 shadow-sm">
                    ...
                    <div>
                        <p className="text-[13px] text-blue-600 font-bold cursor-pointer hover:underline mb-1 flex items-center gap-1">{order.customer.name} <Copy size={12} className="text-slate-300 hover:text-slate-500"/></p>
                        <div className="flex justify-between text-[12px] text-slate-600 mb-1"><span>Tổng chi tiêu (1 đơn hàng)</span><span className="font-bold text-slate-800">0đ</span></div>
                        <div className="flex justify-between text-[12px] text-slate-600"><span>Đơn gần nhất</span><span className="text-blue-600 cursor-pointer hover:underline">{order.id}</span></div>
                    </div>
                    <Separator/>
                    <div className="group relative">
                        <div className="flex justify-between items-center mb-1"><span className="text-[13px] font-bold text-slate-700">Thông tin liên hệ</span><PenLine size={12} className="text-slate-400 cursor-pointer opacity-0 group-hover:opacity-100 hover:text-blue-600"/></div>
                        <p className="text-[12px] text-slate-500 mb-1">{order.customer.email}</p>
                        <p className="text-[13px] text-blue-600 font-medium cursor-pointer hover:underline">{order.customer.phone}</p>
                    </div>
                    <Separator/>
                    <div className="group relative">
                        <div className="flex justify-between items-center mb-1"><span className="text-[13px] font-bold text-slate-700">Địa chỉ giao hàng</span><PenLine size={12} className="text-slate-400 cursor-pointer opacity-0 group-hover:opacity-100 hover:text-blue-600"/></div>
                        <p className="text-[12px] text-slate-800 font-medium mb-1">{order.customer.name} - {order.customer.phone}</p>
                        <p className="text-[12px] text-slate-600 leading-snug">{order.customer.shippingAddress}</p>
                        <div className="flex items-center gap-1 mt-1.5 text-[11px] text-blue-600 cursor-pointer hover:underline"><MapPin size={10} /> Xem bản đồ</div>
                    </div>
                </div>
                <div className="bg-white rounded-sm shadow-sm border border-slate-200 p-4 space-y-3 shadow-sm">
                    ...
                    <div className="group"><div className="flex justify-between items-center mb-1"><p className="text-[12px] font-bold text-slate-600">Nhân viên phụ trách</p><PenLine size={12} className="text-slate-400 cursor-pointer opacity-0 group-hover:opacity-100 hover:text-blue-600"/></div><p className="text-[12px] text-slate-800">{order.info.staff}</p></div>
                    <Separator className="bg-slate-100"/>
                    <div><p className="text-[12px] font-bold mb-1 text-slate-600">Nhân viên tạo đơn</p><p className="text-[12px] text-slate-800">{order.info.creator}</p></div>
                    <Separator className="bg-slate-100"/>
                    <div><p className="text-[12px] font-bold mb-1 text-slate-600">Ngày đặt hàng</p><p className="text-[12px] text-slate-800">{order.createdAt}</p></div>
                    <Separator className="bg-slate-100"/>
                    <div className="group"><div className="flex justify-between items-center mb-1"><p className="text-[12px] font-bold text-slate-600">Ngày hẹn giao</p><PenLine size={12} className="text-slate-400 cursor-pointer opacity-0 group-hover:opacity-100 hover:text-blue-600"/></div><p className="text-[12px] text-slate-400 italic">{order.info.deliveryDate}</p></div>
                    <Separator className="bg-slate-100"/>
                    <div><div className="flex justify-between items-center mb-2"><p className="text-[12px] font-bold text-slate-600">Tag</p><span className="text-[12px] text-blue-600 cursor-pointer hover:underline">Danh sách tag</span></div><Input placeholder="Tìm kiếm hoặc thêm mới tag" className="h-9 text-[12px] bg-white border-slate-200 focus-visible:ring-blue-500 placeholder:text-slate-400"/></div>
                </div>
            </div>
        </div>
      ...
      {showPushShippingModal && (<div className="fixed inset-0 bg-black/50 z-[90] flex items-center justify-center animate-in fade-in duration-200"><div className="bg-white rounded-lg shadow-xl w-[900px] max-w-[95vw] h-[600px] overflow-hidden flex flex-col"><div className="flex justify-between items-center px-6 py-4 border-b border-slate-200"><h2 className="text-[18px] font-bold text-slate-800">Đẩy qua đối tác vận chuyển</h2><button onClick={() => setShowPushShippingModal(false)} className="text-slate-400 hover:text-slate-600"><X size={24}/></button></div><div className="flex flex-1 overflow-hidden"><div className="w-[300px] bg-[#f9fafb] border-r border-slate-200 p-4 space-y-4 overflow-y-auto"><div className="space-y-1"><div className="flex justify-between"><label className="text-[13px] font-bold text-slate-700">Địa chỉ giao hàng</label><Edit size={12} className="text-slate-400 cursor-pointer"/></div><p className="text-[12px] text-slate-600">{order.customer.name}, {order.customer.phone}</p><p className="text-[12px] text-slate-600">{order.customer.shippingAddress}</p></div><div className="space-y-1"><label className="text-[13px] font-bold text-slate-700">Địa chỉ lấy hàng</label><Select defaultValue="main"><SelectTrigger className="h-9 text-[13px] bg-white"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="main">Cửa hàng chính</SelectItem></SelectContent></Select></div><div className="space-y-2"><div className="flex justify-between"><label className="text-[13px] font-bold text-slate-700">Thông tin giao hàng</label><span className="text-[11px] text-blue-600 cursor-pointer">Cấu hình gói hàng</span></div><div className="grid grid-cols-2 gap-2"><div><label className="text-[11px] text-slate-500">Tiền thu hộ COD</label><div className="relative"><Input className="h-8 text-[12px] bg-white pr-6" defaultValue="110,000"/><span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-slate-400">đ</span></div></div><div><label className="text-[11px] text-slate-500">Giá trị hàng hóa</label><div className="relative"><Input className="h-8 text-[12px] bg-white pr-6" defaultValue="110,000"/><span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-slate-400">đ</span></div></div></div></div></div><div className="flex-1 flex flex-col"><div className="px-6 py-3 border-b border-slate-200 flex items-center gap-6"><div className={cn("text-[14px] font-medium cursor-pointer pb-2 relative", shippingTab === 'integrated' ? "text-blue-600" : "text-slate-500")} onClick={() => setShippingTab('integrated')}>Đối tác tích hợp {shippingTab === 'integrated' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600"/>}</div><div className={cn("text-[14px] font-medium cursor-pointer pb-2 relative", shippingTab === 'self' ? "text-blue-600" : "text-slate-500")} onClick={() => setShippingTab('self')}>Tự liên hệ {shippingTab === 'self' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600"/>}</div></div><div className="flex-1 p-6 overflow-y-auto">{shippingTab === 'integrated' ? (<div className="space-y-4">{SHIPPING_PARTNERS_MOCK.map(partner => (<div key={partner.id} className={cn("border rounded-md p-4 flex items-center justify-between transition-all", partner.connected ? "border-blue-200 bg-blue-50/30" : "border-slate-200")}> <div className="flex items-center gap-4"><div className="w-12 h-12 bg-white rounded border border-slate-200 flex items-center justify-center font-bold text-slate-400">{partner.logo}</div><div><p className="text-[14px] font-bold text-slate-800">{partner.name}</p>{partner.error ? (<p className="text-[11px] text-red-500 flex items-center gap-1"><AlertCircle size={10}/> {partner.error}</p>) : (<p className="text-[11px] text-slate-500">Giá cước: <span className="font-bold text-slate-700">30,000đ</span> - Dự kiến: 1-2 ngày</p>)}</div></div>{partner.connected ? (<Button size="sm" className="bg-blue-600 hover:bg-blue-700" onClick={handlePushShipping}>Chọn</Button>) : (<Button size="sm" variant="outline" className="border-blue-600 text-blue-600">Kết nối</Button>)}</div>))}</div>) : (<div className="space-y-4"><div className="space-y-1.5"><label className="text-[13px] font-medium text-slate-600">Đối tác vận chuyển</label><Input placeholder="Nhập tên đối tác" className="h-9 text-[13px]"/></div><div className="space-y-1.5"><label className="text-[13px] font-medium text-slate-600">Mã vận đơn</label><Input placeholder="Nhập mã vận đơn (nếu có)" className="h-9 text-[13px]"/></div><div className="space-y-1.5"><label className="text-[13px] font-medium text-slate-600">Phí vận chuyển</label><div className="relative"><Input defaultValue="0" className="h-9 text-[13px] pr-8"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-[12px]">đ</span></div></div><div className="pt-4"><Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={handlePushShipping}>Đẩy vận chuyển</Button></div></div>)}</div></div></div></div></div>)}
