...
export default function AdminReceiptFormPage() {
    ...
    if (isEditMode && receiptId) {
      const fetchDetail = async () => {
        try {
          ...
          const data = await InventoryApiService.getReceiptDetail(receiptId as string);

          // Điền dữ liệu vào form
          reset({
            ...data,
            // Đảm bảo định dạng ngày yyyy-MM-dd cho input date
            entryDate: data.entryDate ? new Date(data.entryDate).toISOString().slice(0, 10) : "",
          });

          // Cập nhật các state hiển thị ngoài form
  ...
  useEffect(() => {
    const fetchBranches = async () => {
      ...
      try {
        ...
        // Chỉ tự động set branch khi tạo mới, khi sửa thì dùng data từ API
  ...
  // --- EFFECT: SET DELIVERER (CHỈ KHI TẠO MỚI) ---
  useEffect(() => {
    if (user && !isEditMode) {
  ...
  const onSubmit = async (data: Receipt) => {
    ...
    try {
      // Chuẩn hóa payload theo yêu cầu API
      const payload = {
        ...data,
        tags: tags,
      ...
      if (isEditMode) {
        await InventoryApiService.updateReceipt(receiptId as string, payload);
    ...
    >
      <div className="flex items-center gap-4 mb-2 px-1">
        ...
        <h1 className="text-[18px] font-black text-[#1f1f1f] tracking-tight uppercase">
          {isEditMode ? "Cập nhật phiếu nhập hàng" : "Tạo phiếu nhập hàng mới"}
        </h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          ...
          {!selectedSupplier ? (
            <div className="relative mb-2" ref={dropdownRef}>
              ...
              <Input
                ...
                className={`pl-10 h-10 border-[#ccc] rounded-none focus-visible:ring-blue-500/20 shadow-none text-[13px] ${errors.supplierCode ? "border-rose-500" : ""}`}
              />
        ...
        <div className="md:col-span-5 bg-white border border-[#dcdcdc] p-5 rounded-none shadow-sm space-y-3">
          ...
          <div className="grid grid-cols-12 items-center gap-2">
            ...
            <div className="col-span-8 relative">
              <Input type="date" {...register("entryDate")} className={`h-9 text-[13px] border-[#ccc] rounded-none shadow-none font-medium pr-10 ${errors.entryDate ? "border-rose-500" : ""}`} />
                ...
                <div className="absolute top-full left-0 w-full mt-1 bg-white border border-slate-200 shadow-xl z-50 rounded-sm overflow-hidden flex flex-col">
                  ...
                  {checkedItems.length > 0 && (
                    <div className="p-3 bg-blue-50 border-t flex justify-between items-center">
                      <span className="text-[12px] font-bold text-blue-700">Đã chọn {checkedItems.length} sản phẩm</span>
                  ...
                  return (
                    <React.Fragment key={field.id}>
                      <TableRow className="border-b hover:bg-slate-50/30 transition-colors">
                        ...
                        <TableCell className="p-3">
                          <Input {...register(`items.${index}.lotNumber`)} className={`h-8 text-[12px] border-[#ccc] rounded-none text-center font-bold ${errors.items?.[index]?.lotNumber ? "border-rose-500" : ""}`} />
                        </TableCell>
                        <TableCell className="p-3">
                          <Input type="date" {...register(`items.${index}.expiryDate`)} className={`h-8 text-[12px] border-[#ccc] rounded-none ${errors.items?.[index]?.expiryDate ? "border-rose-500" : ""}`} />
                        </TableCell>
                        <TableCell className="p-3">
                          <Input type="number" {...register(`items.${index}.plannedQuantity`, { valueAsNumber: true })} className={`h-8 text-[12px] text-right font-bold ${errors.items?.[index]?.plannedQuantity ? "border-rose-500" : ""}`} />
                        </TableCell>
                        <TableCell className="p-3">
                          <Input type="number" {...register(`items.${index}.importPrice`, { valueAsNumber: true })} className={`h-8 text-[12px] text-right font-bold text-blue-600 ${errors.items?.[index]?.importPrice ? "border-rose-500" : ""}`} />
                        </TableCell>
                        <TableCell className="p-3">
                          <Input type="number" {...register(`items.${index}.newSellingPrice`, { valueAsNumber: true })} className="h-8 text-[12px] text-right font-bold text-emerald-700 bg-emerald-50/30" />
                        </TableCell>
                        ...
                        <TableCell className="p-3 text-center"><button type="button" onClick={() => remove(index)} className="text-slate-300 hover:text-rose-600"><Trash2 size={16} /></button></TableCell>
                      </TableRow>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pt-6">
        <div className="md:col-span-5 space-y-6">
          <div className="space-y-2">
            ...
            <textarea {...register("note")} className="w-full min-h-[90px] p-3 text-[13px] border border-[#ccc] outline-none shadow-inner" placeholder="Nhập ghi chú..."></textarea>
          </div>
          <div className="space-y-2">
            <Label className="text-[12px] font-bold text-slate-500 uppercase">Thẻ phân loại (Tags)</Label>
            <div className="min-h-[44px] p-2 border border-[#ccc] bg-white flex flex-wrap gap-2 shadow-inner">
              {tags.map((tag) => (
                <span key={tag} className="bg-slate-100 px-2 py-1 text-[11px] font-bold border flex items-center gap-1">
                  {tag} <X size={10} className="cursor-pointer" onClick={() => setTags(tags.filter((t) => t !== tag))} />
                </span>
              ))}
              <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={handleAddTag} placeholder="Nhập tag..." className="flex-1 min-w-[150px] outline-none text-[12px]" />
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-4 md:col-start-9 space-y-4 pr-2">
          ...
          <div className="pt-4 border-t">
            ...
            <div className="relative mt-2">
              <Input type="number" {...register("paymentAmount", { valueAsNumber: true })} className="h-10 text-[16px] font-black text-blue-600 border-[#ccc] text-right bg-blue-50/30 shadow-none" />
            </div>
          </div>
      ...
      <div className="fixed bottom-0 left-0 lg:left-[260px] right-0 bg-[#f8f9fa] border-t p-[12px_30px] flex items-center justify-end gap-[15px] z-[999] shadow-[0_-4px_15px_rgba(0,0,0,0.05)]">
        <Button type="button" variant="outline" onClick={() => router.back()} className="min-w-[110px] text-[12px] font-bold uppercase border-[#ccc] bg-white rounded-none">Hủy bỏ</Button>
        <Button
          type="submit"
