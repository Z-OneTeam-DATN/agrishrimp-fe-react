...
export default function AdminDashboard() {
  ...
  const getActivityConfig = (type: string) => {
    switch (type) {
      case 'ORDER':
        return { icon: ShoppingCart, iconColor: "text-blue-500", bgColor: "bg-blue-50" };
      case 'INVENTORY':
        return { icon: Package, iconColor: "text-emerald-500", bgColor: "bg-emerald-50" };
      case 'CUSTOMER':
        return { icon: Users, iconColor: "text-indigo-500", bgColor: "bg-indigo-50" };
      default:
        return { icon: AlertTriangle, iconColor: "text-amber-500", bgColor: "bg-amber-50" };
    }
  };
  ...
  if (userError) return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 text-center">
      ...
      <h2 className="text-xl font-bold text-slate-800">Không thể tải dữ liệu người dùng</h2>
      <p className="text-sm text-slate-500 max-w-md mt-2">Đã xảy ra lỗi khi kết nối với máy chủ. Vui lòng kiểm tra lại kết nối hoặc đăng nhập lại.</p>
    ...
    <div className="space-y-4 pb-10 bg-gray-50/50 min-h-screen p-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
        <div className="flex items-center gap-4">
          <div>
            <h1 className="text-xl font-black text-slate-800 uppercase tracking-tight">Tổng quan hệ thống</h1>
            <p className="text-xs text-slate-500">Dữ liệu kinh doanh cập nhật theo thời gian thực</p>
          </div>
          <button 
          ...
          >
            <RefreshCw size={16} className={cn("text-gray-500 group-hover:text-blue-600", isRefreshing && "animate-spin text-blue-600")} />
          </button>
        </div>
        ...
        {isAdmin ? (
          <div className="flex items-center gap-3 bg-white p-2 rounded-sm border border-gray-200 shadow-sm w-full md:w-auto">
            ...
            <div className="flex-1 md:w-64">
              <p className="text-[10px] font-bold text-gray-400 uppercase leading-none mb-1">Chi nhánh hiển thị</p>
              <Select 
                value={selectedBranchId || "all"} 
                onValueChange={(val) => setSelectedBranchId(val === "all" ? undefined : val)}
              >
                <SelectTrigger className="h-7 text-xs border-none p-0 focus:ring-0 shadow-none font-bold text-slate-700">
                  <SelectValue placeholder="Tất cả chi nhánh" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">Tất cả chi nhánh (Tổng hợp)</SelectItem>
                  {branches.map((b: any) => (
          ...
          <div className="flex items-center gap-3 bg-white p-2 px-4 rounded-sm border border-gray-200 shadow-sm">
            ...
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase leading-none mb-1">Đang xem tại</p>
              <p className="text-xs font-bold text-slate-700">{user?.branch?.name || "Chi nhánh của bạn"}</p>
            </div>
          </div>
        )}
      </div>
              ...
              {isRecentLoading
                ...
                : isRecentError ? (
                  <div className="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p className="text-xs text-gray-500">Không thể tải hoạt động</p>
                    <button onClick={() => refetchRecent()} className="text-[10px] text-blue-500 font-bold mt-2 hover:underline">Thử lại</button>
                  </div>
                ) : recentActivities.length === 0 ? (
                  <div className="text-center py-10">
                    <p className="text-xs text-gray-400">Không có hoạt động nào</p>
                  </div>
                ) : (
                  recentActivities.map((activity, index) => {
                    ...
                    return (
                      <div key={activity.id} className="flex gap-3 relative group">
                        ...
                        <div className={`${config.bgColor} ${config.iconColor} p-2 rounded-full h-8 w-8 flex items-center justify-center shrink-0 z-10`}>
                          <config.icon size={14} />
                        </div>
