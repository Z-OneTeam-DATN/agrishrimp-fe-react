...
import { Upload, X, Loader2, Copy } from "lucide-react";
...
export default function EditDriverPage() {
    ...
    const { register, handleSubmit, control, watch, setValue, reset, formState: { errors } } = useForm<DriverFormValues>({
    ...
    const onSubmit = async (data: DriverFormValues) => {
        ...
        try {
            await driverService.update(driverId, data);
                ...
                <div className="rounded-[4px] border border-slate-200 bg-white p-6 shadow-sm space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-5 gap-y-6">
                        <div className="space-y-1.5">
                            <Label className="text-[10.5px] font-semibold text-slate-500">Mã tài xế</Label>
                            <div className="relative flex items-center">
                                <Input
                                    {...register("code")}
                                    disabled
                                    className="h-10 pr-9 text-[13px] font-normal rounded-[4px] border-slate-200 bg-slate-50 text-slate-500 cursor-not-allowed shadow-none"
                                />
                                <Button
                                    type="button"
                                    variant="ghost"
                                    size="icon"
                                    className="absolute right-1.5 h-7 w-7 text-slate-400 hover:text-emerald-600 rounded-[4px]"
                                    onClick={() => copyValue("Mã tài xế", watch("code"))}
                                >
                                    <Copy size={14} />
                                </Button>
                            </div>
                    ...
                    {/* Audit Info displayed as secondary text at the bottom of the section */}
                    {auditInfo && (
                        <div className="pt-4 border-t border-slate-100 text-[10px] text-slate-400 space-y-0.5 font-normal">
                            {auditInfo.createdAt && (
                                <p>
                                    Ngày tạo: {formatAuditDate(auditInfo.createdAt)}
                                    {auditInfo.createdByName ? ` bởi ${auditInfo.createdByName}` : ""}
                                </p>
                            )}
                            {auditInfo.updatedAt && (
                                <p>
                                    Cập nhật lần cuối: {formatAuditDate(auditInfo.updatedAt)}
                                    {auditInfo.updatedByName ? ` bởi ${auditInfo.updatedByName}` : ""}
                                </p>
                            )}
                        </div>
                    )}
