...
import { Attribute, ProductDetail, ProductVariant, UpdateProductRequest, UpdateProductVariantRequest } from "@/app/types/product.schema";
import { z } from "zod";
...
export default function EditProductPage() {
    ...
    if (!isLoadingAuth && isAuthenticated && id) {
      const fetchData = async () => {
        try {
          ...
          console.log(">>> [BE Data] Danh mục:", catRes);
          console.log(">>> [BE Data] Thuộc tính hệ thống:", attrRes);
          console.log(">>> [BE Data] Chi tiết sản phẩm:", productDetail);
          if (productDetail.variants && productDetail.variants.length > 0) {
            console.log(">>> [BE Data] Chi tiết Biến thể 0:", productDetail.variants[0]);
          }
          ...
          // Reset form với dữ liệu sản phẩm chuẩn hóa từ cấu trúc BE thực tế
          const mappedData: ProductFormData = {
            name: productDetail.name || "",
            // Ưu tiên lấy ID từ category object, sau đó mới đến categoryId trực tiếp
            categoryId: productDetail.category?.id ? String(productDetail.category.id) : 
                        ((productDetail as any).categoryId ? String((productDetail as any).categoryId) : ""),
            
            // Ưu tiên lấy tên từ brand object, sau đó mới đến brandName hoặc brand trực tiếp
            ...
            status: (productDetail.status as any) || "ACTIVE",
            variants: (productDetail.variants || []).map((v: any) => {
              // Trích xuất ID giá trị thuộc tính từ mảng attributeValues
              let attrIds: number[] = [];
              if (Array.isArray(v.attributeValues) && v.attributeValues.length > 0) {
                // Ưu tiên valueId hoặc attributeValueId (thường là ID của giá trị thuộc tính)
                // Hạn chế dùng .id vì nó có thể là ID của bản ghi liên kết (junction table ID)
                attrIds = v.attributeValues
                  .map((av: any) => av.valueId || av.attributeValueId || av.id)
                  .filter((id: any) => id !== null && id !== undefined)
                  .map((id: any) => Number(id));
                console.log(`>>> [Mapping Variant ${v.sku}] attributeValues raw:`, v.attributeValues);
                console.log(`>>> [Mapping Variant ${v.sku}] Extracted attrIds:`, attrIds);
              } else if (Array.isArray(v.attributeValueIds)) {
                attrIds = v.attributeValueIds.map((id: any) => Number(id));
                console.log(`>>> [Mapping Variant ${v.sku}] attributeValueIds:`, attrIds);
              }
              ...
              return {
                ...
                shippingWeight: v.shippingWeight || 0,
          ...
          console.log(">>> [Form Mapping] Dữ liệu sau khi mapping vào form:", mappedData);
          reset(mappedData);

          // Cập nhật các state quản lý ảnh (ngoài react-hook-form)
          ...
          // Set unit conversions (if any from first variant or somewhere)
          if (productDetail.variants?.[0]?.unitConversions) {
            setUnitConversions(productDetail.variants[0].unitConversions.map(uc => ({
  ...
  const setAttributeValue = (
    ...
    // Lấy danh sách tất cả ID giá trị của thuộc tính này để loại bỏ các ID cũ
    const attrValueIds = attr.values.map(v => {
      const vid = v?.id ?? v?.valueId ?? v;
      return Number(vid);
    });
  ...
  const uploadFile = async (file: File): Promise<string> => {
    ...
    const res = await FileService.tmpUpload(formData);
    return res.tmpPath;
  };
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      ...
      // Upload main images if needed
      const mainImageUrls = await Promise.all(
        productImageFiles.map(async (img) => {
      ...
      const variantImageUrls = await Promise.all(
        variantImageFiles.map(async (img) => {
          if (!img) return null;
          if (typeof img === "string") return img;
          return await uploadFile(img);
        })
      ...
      toast.error(res?.message || "Có lỗi khi cập nhật sản phẩm.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
                      ...
                      <div className="flex-1 space-y-4">
                        {attributes.length > 0 && (
                          <div
                            className={cn(
                              "grid gap-4",
                              attributes.length === 1 ? "grid-cols-1 max-w-xs" : attributes.length === 2 ? "grid-cols-2" : "grid-cols-3"
                            )}
                          >
                            {attributes.map((attr) => {
                              // Tìm ID đã chọn cho thuộc tính này
                              // Chúng ta duyệt qua tất cả ID của variant, xem ID nào thuộc về thuộc tính (attr) này
                              const selectedValueId =
                                variantAttributeIds.find((id: number | string) =>
                                  attr.values.some((v: any) => {
                                    const vid = v?.id ?? v?.valueId ?? (typeof v === 'object' ? null : v);
                                    // So sánh lỏng (==) hoặc ép kiểu để chắc chắn khớp
                                    return vid !== null && Number(vid) === Number(id);
                                  })
                                ) ?? "none";
                              
                              if (idx === 0) {
                                const availableIds = attr.values.map((v: any) => v.id ?? v.valueId ?? v);
                                console.log(`>>> [Render Debug] Attr: ${attr.name} (ID: ${attr.id})`);
                                console.log(`    - variantAttributeIds của variant:`, variantAttributeIds);
                                console.log(`    - Các IDs khả dụng trong hệ thống cho Attr này:`, availableIds);
                                console.log(`    - Kết quả selectedValueId:`, selectedValueId);
                              }
                                  ...
                                  >
                                    ...
                                    <SelectContent className="rounded-none">
                                      ...
                                      {attr.values.map((v, vIdx) => {
                                        const vid = v?.id ?? v?.valueId ?? v;
                                        const vlabel = v?.value ?? v?.name ?? String(v);
                                        return (
                                          <SelectItem
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              className="h-[34px] border-[#ccc] rounded-none text-right font-bold text-blue-600 bg-blue-50/20 shadow-none"
                            />
                          ...
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              className="h-[34px] border-[#ccc] rounded-none text-right font-bold text-emerald-700 shadow-none"
                            />
                          ...
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              className="h-[34px] border-[#ccc] rounded-none text-right font-bold text-orange-500 shadow-none"
                            />
                          </div>
                        </div>
              ...
              <div className="space-y-2">
                {unitConversions.map((uc, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <span className="text-[11px] font-bold text-slate-400 w-4 text-right shrink-0">1</span>
                    <Input
                      value={uc.fromUnit}
                      onChange={(e) => updateUnitConversion(idx, "fromUnit", e.target.value)}
                    ...
                    <Input
                      type="number"
                      value={uc.rate}
                      onChange={(e) => updateUnitConversion(idx, "rate", e.target.value)}
                      placeholder="SL"
                      className="h-[34px] border-[#ccc] rounded-none text-[13px] shadow-none w-[72px] text-right"
                    />
                    <Input
                      value={uc.toUnit}
                      onChange={(e) => updateUnitConversion(idx, "toUnit", e.target.value)}
          ...
          <div className="bg-white border border-[#dcdcdc] p-5 rounded-none shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              {productImagePreviews.map((src, i) => (
                <div key={i} className="relative aspect-square border border-[#eee] group overflow-hidden">
              ...
              >
                ...
                <span className="text-[8px] font-black text-slate-400 group-hover:text-emerald-600 uppercase">Tải thêm</span>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 lg:left-[260px] right-0 bg-white border-t border-[#ddd] p-[12px_30px] flex items-center justify-end gap-3 z-[999] shadow-[0_-4px_15px_rgba(0,0,0,0.05)]">
        ...
        >
          {isLoading ? <Loader2 size={17} className="mr-2 animate-spin" /> : <Save size={17} className="mr-2" />}
          Lưu thay đổi
        </Button>
      </div>
    </form>
