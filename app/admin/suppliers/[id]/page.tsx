...
export default function SupplierDetailPage() {
      ...
      try {
        ...
        if (supplierInfo) {
          reset({
            ...
            paymentTerms: (supplierInfo.paymentTerm?.toLowerCase() as SupplierFormValues['paymentTerms']) || '',
  ...
  if (isLoading) return <div className="flex h-screen items-center justify-center text-sm text-gray-500">Đang tải dữ liệu...</div>;
          ...
          <div className="bg-white border border-[#dcdcdc] rounded-[4px] shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-50 flex flex-col items-center text-center">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 w-full">
                ...
                <div className="bg-slate-50 p-2 rounded text-center">
                  ...
                  <p className={cn("text-[10px] font-bold uppercase", supplierData.status === 'active' ? "text-emerald-600" : "text-rose-500")}>
                    {supplierData.status === 'active' ? "Đang giao dịch" : "Tạm ngừng"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex items-start gap-3 pt-3 border-t border-dashed">
                ...
                <div className="flex flex-col w-full">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Nhóm hàng</span>
                  <Controller
                    name="category"
                    control={control}
                    render={({ field }) => (
            ...
            <TabsContent value="info" className="space-y-4 mt-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white border border-[#dcdcdc] p-6 rounded-none shadow-sm">
                    ...
                    <Textarea {...register("note")} placeholder="Nhập các lưu ý quan trọng..." className="min-h-[100px] text-[13px] border-[#ccc] rounded-none shadow-none focus:border-emerald-500" />
                  </div>
              </div>
              ...
              <div className="bg-white border border-[#dcdcdc] rounded-[4px] shadow-sm p-4">
                 <h3 className="text-[12px] font-black text-slate-700 uppercase flex items-center gap-2 mb-4 border-b pb-2">
                    <Plus size={14} className="text-blue-500" /> Cấu hình tài chính
                 </h3>
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-1">
                        <Label className="text-[10px] font-bold text-slate-500 uppercase">Chu kỳ thanh toán</Label>
                        <Controller
                            name="paymentTerms"
                            control={control}
                            render={({ field }) => (
                                <Select onValueChange={field.onChange} value={field.value}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="net30">Công nợ 30 ngày</SelectItem>
                    ...
                    <div className="space-y-1">
                        <Label className="text-[10px] font-bold text-slate-500 uppercase">Hạn mức nợ</Label>
                        <Input {...register("creditLimit")} type="number" className="h-[30px] text-[12px]" />
                    </div>
                    <div className="space-y-1">
                        <Label className="text-[10px] font-bold text-slate-500 uppercase">Chiết khấu (%)</Label>
                        <Input {...register("discount")} type="number" className="h-[30px] text-[12px]" />
                    </div>
                 </div>
              </div>
            </TabsContent>
                    ...
                    <TableBody>
                      {importHistory.map((item) => (
                        <TableRow key={item.id} className="border-b border-slate-50 hover:bg-emerald-50/20">
                          <TableCell className="text-[12px] font-black text-emerald-600 pl-4">#{item.id}</TableCell>
                          <TableCell><span className="text-[9px] font-bold border px-1 rounded">{item.status}</span></TableCell>
