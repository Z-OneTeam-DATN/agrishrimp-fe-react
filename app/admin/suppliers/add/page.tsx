...
export default function AddSupplierPage() {
  ...
  const handleLookupBank = async () => {
    ...
    if (!accountNumber || !bankBin) {
      toast.error("Vui lòng chọn ngân hàng và nhập số tài khoản trước khi xác thực");
      return;
    }

    const loadingBank = toast.loading("Đang xác thực tài khoản ngân hàng...");
    try {
      const ownerName = await supplierService.lookupBank(bankBin, accountNumber);
      if (ownerName) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="bg-white border border-[#dcdcdc] p-6 rounded-none shadow-sm">
             ...
             <div className="space-y-6">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                 <div className="space-y-1.5">
                   <Label className="text-[10px] font-black text-slate-500 uppercase tracking-tight">Chu kỳ thanh toán *</Label>
                   <Controller
                    name="paymentTerms"
                    control={control}
                    render={({ field }) => (
                 ...
                 <div className="space-y-1.5">
                   <Label className="text-[10px] font-black text-rose-600 uppercase tracking-tight">Hạn mức công nợ (₫)</Label>
                   <Input {...register("creditLimit")} type="number" className="h-[34px] text-[13px] border-[#ccc] rounded-none text-right font-bold text-rose-600 bg-rose-50/10" />
                 </div>
                 <div className="space-y-1.5">
                   <Label className="text-[10px] font-black text-emerald-600 uppercase tracking-tight">Chiết khấu (%)</Label>
                   <Input {...register("discount")} type="number" className="h-[34px] text-[13px] border-[#ccc] rounded-none text-right font-bold text-emerald-600 bg-emerald-50/10" />
                 </div>
               </div>
               <div className="pt-4 border-t border-dashed border-slate-100">
                 <p className="text-[10px] font-black text-slate-400 uppercase mb-4 italic">Thông tin tài khoản thụ hưởng</p>
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                   <div className="space-y-1.5">
                     <Label className="text-[10px] font-black text-slate-500 uppercase tracking-tight">Ngân hàng *</Label>
                     <Controller
                      name="bankName"
                      control={control}
                      render={({ field }) => (
                   ...
                   <div className="space-y-1.5">
                     ...
                     <div className="flex gap-0">
                       ...
                       <Button type="button" variant="outline" onClick={handleLookupBank} className="h-[34px] bg-slate-50 border-[#ccc] border-l-0 rounded-none px-3 text-[10px] font-black text-blue-600 hover:bg-blue-50">
                   ...
                   <div className="space-y-1.5">
                     ...
                     <Input {...register("bankAccountHolder")} readOnly className="h-[34px] text-[13px] border-[#ccc] rounded-none shadow-none font-bold uppercase bg-slate-50" />
                   </div>
                 </div>
               </div>
             </div>
        ...
        <div className="lg:col-span-3 space-y-5">
           ...
           <div className="bg-white border border-[#dcdcdc] p-6 rounded-none shadow-sm">
             ...
             <Textarea {...register("note")} placeholder="Lưu ý quan trọng..." className="min-h-[150px] text-[13px] border-[#ccc] rounded-none shadow-none" />
           </div>
        </div>
      </div>
