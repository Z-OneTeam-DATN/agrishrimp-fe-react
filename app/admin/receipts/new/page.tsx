...
import { 
  X, Search, ChevronDown, ChevronLeft, Info, Filter, Settings, 
  Plus, Trash2, Package, HelpCircle, Save, AlertCircle, FileText, ShoppingCart,
...
export default function AdminNewReceiptPage() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-6 bg-slate-50 min-h-screen space-y-4 pb-[100px] font-sans text-slate-900">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="md:col-span-7 bg-white border border-[#dcdcdc] p-5 rounded-none shadow-sm">
          <h2 className="text-[13px] font-bold text-slate-700 mb-4 border-b pb-2">Thông tin nhà cung cấp</h2>
        ...
        <div className="md:col-span-5 bg-white border border-[#dcdcdc] p-5 rounded-none shadow-sm space-y-3">
                ...
                render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent className="rounded-none">
                      <SelectItem value="Kho Tổng (Trụ sở)">Kho Tổng (Trụ sở)</SelectItem>
          ...
          <div className="grid grid-cols-12 items-center gap-2">
            ...
            <div className="col-span-8 relative">
              <Input type="date" {...register("entryDate")} className="h-9 text-[13px] border-[#ccc] rounded-none shadow-none font-medium" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex flex-wrap items-center gap-3 flex-1">
            ...
            <div className="flex h-10 items-center">
              <Button type="button" variant="outline" className="h-full text-[12px] border-[#ccc] rounded-none px-3 shadow-none border-r-0 font-bold hover:bg-slate-50">
                ...
                fields.map((field, index) => (
                  <TableRow key={field.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/30 transition-colors group">
                    ...
                    <TableCell className="p-3 text-center">
                      <button type="button" onClick={() => remove(index)} className="text-slate-300 hover:text-rose-600 transition-colors">
                        <Trash2 size={16} />
                      </button>
                    </TableCell>
                  </TableRow>
            ...
            <div className="min-h-[44px] p-2 border border-[#ccc] rounded-none bg-white flex flex-wrap gap-2 shadow-inner">
              {tags.map(tag => (
                <span key={tag} className="inline-flex items-center gap-1 bg-slate-100 text-slate-600 px-2 py-1 text-[11px] font-bold border border-slate-200">
                  {tag}
                  <X size={10} className="cursor-pointer hover:text-rose-500" onClick={() => setTags(tags.filter(t => t !== tag))} />
                </span>
              ))}
              <input 
                type="text" 
