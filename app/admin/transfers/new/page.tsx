  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pb-[100px] bg-slate-50/30 p-4 min-h-screen">
              ...
              render={({ field }) => (
                <RadioGroup onValueChange={field.onChange} value={field.value} className="flex items-center gap-6">
                  <div className="flex items-center space-x-2">
                    ...
                    <Label htmlFor="type-wh" className={cn("text-[11px] font-bold uppercase tracking-wider cursor-pointer", field.value === "BETWEEN_WAREHOUSES" ? "text-blue-600" : "text-slate-400")}>Điều chuyển liên kho</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="INTERNAL" id="type-internal" />
                    <Label htmlFor="type-internal" className={cn("text-[11px] font-bold uppercase tracking-wider cursor-pointer", field.value === "INTERNAL" ? "text-blue-600" : "text-slate-400")}>Nội bộ</Label>
                  </div>
                </RadioGroup>
              )}
            />
        ...
        <div className="flex items-center justify-between max-w-4xl mx-auto">
          {steps.map((step, idx) => (
            <React.Fragment key={idx}>
              ...
              {idx < steps.length - 1 && <div className="flex-1 h-[3px] bg-slate-100 mx-2 -mt-6 relative"><div className={cn("absolute inset-0 transition-all duration-500", steps[idx].status === "completed" ? "bg-emerald-500" : "bg-transparent")} /></div>}
            </React.Fragment>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="bg-white border border-[#dcdcdc] p-6 rounded-none shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-x-6 gap-y-5">
              <div className="md:col-span-8 space-y-1.5">
                ...
                <Input {...register("description")} className="h-[34px] text-[13px] border-[#ccc] rounded-none font-bold focus:border-blue-500 shadow-none" />
              </div>
                ...
                <React.Fragment>
                  <div className="md:col-span-4 space-y-1.5 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    <div className="relative">
                      ...
                      <Input {...register("vehicle")} className="h-[34px] pl-9 text-[13px] border-[#ccc] rounded-none focus:border-blue-500 shadow-none" placeholder="Biển số xe..." />
                    </div>
                  </div>
                  <div className="md:col-span-4 space-y-1.5 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    <div className="relative">
                      ...
                      <Input {...register("transporter")} className="h-[34px] pl-9 text-[13px] border-[#ccc] rounded-none focus:border-blue-500 shadow-none" placeholder="Họ tên tài xế..." />
                    </div>
                  </div>
                  <div className="md:col-span-4 space-y-1.5 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    <div className="relative">
                      ...
                      <Input {...register("dispatchOrder")} className="h-[34px] pl-9 text-[13px] border-[#ccc] rounded-none font-mono focus:border-blue-500 shadow-none" placeholder="Số hiệu văn bản..." />
                    </div>
                  </div>
                </React.Fragment>
              )}
          ...
          <div className="bg-white border border-[#dcdcdc] rounded-none shadow-sm overflow-hidden">
              ...
              <Table className="table-custom border-collapse min-w-[1250px]">
                <TableHeader>
                  <TableRow className="bg-slate-50 border-b border-[#ccc]">
                    ...
                    <TableHead className="w-[100px] p-2 text-[10px] font-black uppercase text-slate-500">Quy đổi</TableHead>
                ...
                <TableBody>
                  {fields.map((field, index) => (
                    <TableRow key={field.id} className="border-b border-slate-100 hover:bg-blue-50/20 transition-colors">
                      ...
                      <TableCell className="p-1"><Input {...register(`items.${index}.productName`)} className="h-8 text-[12px] border-none bg-transparent font-bold focus:ring-0" /></TableCell>
                      <TableCell className="p-1"><Input {...register(`items.${index}.unit`)} className="h-8 text-[12px] border-none bg-transparent text-center focus:ring-0" /></TableCell>
                      <TableCell className="p-1 text-center text-[10px] font-medium text-slate-400 italic">1T = 12C</TableCell>
                      <TableCell className="p-1 text-right font-bold text-slate-500 pr-3">{watch(`items.${index}.availableQuantity`)}</TableCell>
                      <TableCell className="p-1"><Input type="number" {...register(`items.${index}.quantity`)} className="h-8 text-[13px] text-right border-blue-200 bg-blue-50/30 rounded-none font-black text-blue-700 focus:border-blue-500 focus:ring-0" /></TableCell>
                      <TableCell className="p-1"><Input type="number" {...register(`items.${index}.receivedQuantity`)} readOnly className="h-8 text-[13px] text-right border-emerald-100 bg-emerald-50/30 rounded-none text-emerald-700 font-bold focus:ring-0" /></TableCell>
                      <TableCell className="p-1"><Input {...register(`items.${index}.fromLoc`)} className="h-8 text-[11px] border-slate-200 rounded-none focus:border-blue-500 focus:ring-0" placeholder="A-01..." /></TableCell>
                      <TableCell className="p-1"><Input {...register(`items.${index}.toLoc`)} className="h-8 text-[11px] border-slate-200 rounded-none focus:border-blue-500 focus:ring-0" placeholder="B-05..." /></TableCell>
                      <TableCell className="p-1"><Input {...register(`items.${index}.itemNote`)} className="h-8 text-[11px] border-none italic bg-transparent focus:ring-0" placeholder="..." /></TableCell>
                      <TableCell className="p-1 text-center"><button type="button" onClick={() => remove(index)} className="text-slate-300 hover:text-rose-500 transition-colors"><Trash2 size={16} /></button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            ...
            {fields.length === 0 && (
              <div className="py-16 flex flex-col items-center justify-center bg-white space-y-4">
                ...
                <div className="text-center"><p className="text-[13px] font-black text-slate-600 uppercase tracking-widest">Danh mục điều chuyển đang trống</p></div>
                <div className="flex gap-3"><Button type="button" onClick={addNewItem} className="bg-blue-600 hover:bg-blue-700 text-white font-black text-[11px] uppercase rounded-none shadow-lg shadow-blue-100 flex gap-2"><ListPlus size={18} /> Chọn hàng từ danh sách</Button></div>
              </div>
            )}
          </div>
        ...
        <div className="lg:col-span-3 space-y-5">
          <div className="bg-white border border-[#dcdcdc] p-6 rounded-none shadow-sm space-y-6">
            <div className="space-y-4">
              <div className="space-y-1.5">
                <Label className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-2"><Building2 size={12} /> Chi nhánh xuất hàng</Label>
                <Select defaultValue="cn-hn"><SelectTrigger className="h-8 text-[12px] border-[#eee] rounded-none font-bold focus:ring-0"><SelectValue /></SelectTrigger><SelectContent className="rounded-none"><SelectItem value="cn-hn">CHI NHÁNH MIỀN BẮC</SelectItem></SelectContent></Select>
              </div>
              <div className="space-y-1.5">
                ...
                <Controller name="sourceAddress" control={control} render={({ field }) => (
                  <Textarea {...field} placeholder="Địa chỉ chi tiết..." className="min-h-[60px] text-[12px] border-[#ccc] rounded-none focus:border-blue-500 shadow-none resize-none bg-slate-50/50" />
                )} />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <Label className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-2"><Building2 size={12} /> Chi nhánh nhận hàng</Label>
                <Select defaultValue="cn-hcm"><SelectTrigger className="h-8 text-[12px] border-[#eee] rounded-none font-bold focus:ring-0"><SelectValue /></SelectTrigger><SelectContent className="rounded-none"><SelectItem value="cn-hcm">CHI NHÁNH MIỀN NAM</SelectItem></SelectContent></Select>
              </div>
              <div className="space-y-1.5">
          ...
          <div className="bg-white border border-[#dcdcdc] p-6 rounded-none shadow-sm space-y-4">
            <div className="space-y-1.5"><Label className="text-[9px] font-bold text-slate-400 uppercase">Ngày điều chuyển (24H)</Label><Input type="datetime-local" {...register("transferDate")} className="h-[34px] text-[12px] border-[#ccc] rounded-none focus:border-blue-500" /></div>
            <div className="space-y-1.5"><Label className="text-[9px] font-bold text-slate-400 uppercase">Tham chiếu chứng từ</Label><Input {...register("referenceCode")} className="h-[34px] text-[12px] border-[#ccc] rounded-none font-mono focus:border-blue-500" placeholder="Mã YCDC, Mã ĐH..." /></div>
          </div>

          <div className="p-5 bg-amber-50 border border-amber-100 rounded-none flex gap-3"><AlertCircle size={18} className="text-amber-600 shrink-0" /><p className="text-[11px] text-amber-700 leading-relaxed font-bold italic uppercase tracking-tighter">Lưu ý: Tồn khả dụng sẽ được cập nhật ngay sau khi xác nhận xuất kho.</p></div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 lg:left-[260px] right-0 bg-[#f8f9fa] border-t border-[#ddd] p-[12px_30px] flex items-center justify-end gap-[15px] z-[999] shadow-[0_-4px_15px_rgba(0,0,0,0.05)]">
        ...
        <Button type="submit" className="min-w-[180px] h-[38px] text-[12px] font-black bg-blue-600 hover:bg-blue-700 text-white rounded-none shadow-md shadow-blue-100 uppercase transition-all active:scale-[0.98]"><Save size={18} className="mr-2" /> LƯU & GỬI DUYỆT</Button>
      </div>
    </form>
  );
}
