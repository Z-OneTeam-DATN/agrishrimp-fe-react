...
export default function AddCustomerPage() {
    ...
    const loadStaffByBranch = async (branchId: string) => {
        try {
            ...
            setStaffList(list);
            // Auto-assign: set logged-in user if they appear in the staff list
            if (user?.id) {
                const matched = list.find((s) => s.id === user.id);
                if (matched) {
                    setValue("staffAssignedId", matched.id.toString());
                }
            }
        } catch (error) {
    ...
    const handleRestoreDraft = () => {
        if (pendingDraftData?.formData) {
            const data = pendingDraftData.formData;
    ...
    return (
        ...
        >
                    ...
                    <div className="bg-white border border-slate-200 p-6 rounded-[4px] shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-5 gap-y-6">
                            ...
                            <div className="space-y-1.5">
                                ...
                                />
                                {errors.gender && <p className="text-[10px] text-red-500 flex items-center gap-1"><AlertCircle size={12} /> {errors.gender.message}</p>}
                            </div>
                            <div className="space-y-1.5">
                                ...
                                />
                                {errors.status && <p className="text-[10px] text-red-500 flex items-center gap-1"><AlertCircle size={12} /> {errors.status.message}</p>}
                            </div>
                        </div>
                    </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-[999] border-t border-slate-200 bg-white px-4 py-3 lg:left-[260px] flex items-center justify-end gap-3">
                ...
                {draftSaved && (
                    <span className="text-[11px] text-emerald-600 font-medium mr-auto flex items-center gap-1.5 animate-in fade-in slide-in-from-left-2 duration-300">
                ...
                <DialogContent className="sm:max-w-[560px] bg-white">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-[13px] text-slate-500">
                            Hệ thống phát hiện bạn có bản nháp chưa hoàn tất cho form thêm khách hàng.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-3">
                        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                            <p className="text-[12px] text-slate-600">
                                Nháp hiện tại: {pendingDraftData?.savedAt ? new Date(pendingDraftData.savedAt).toLocaleString("vi-VN") : "Không rõ thời gian"}
                            </p>
                        </div>
                    </div>
