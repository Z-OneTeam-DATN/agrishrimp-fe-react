...
export default function AddCustomerPage() {
  ...
  const handleAddressChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    typingTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const cleanDName = rawDName
          .replace(/^(Quận|Huyện|Thị xã|Thành phố)\s+/i, "")
          .trim();
        const cleanWName = rawWName
          .replace(/^(Phường|Xã|Thị trấn)\s+/i, "")
          .trim();

        const contextArr = [cleanWName, cleanDName, cleanPName].filter(Boolean);
        const searchQuery =
          contextArr.length > 0 ? `${value}, ${contextArr.join(", ")}` : value;
        ...
        if (data.results && data.results.length > 0) {
          ...
          const normalizedSelectedDistrict = removeAccents(cleanDName);
          const normalizedSelectedWard = removeAccents(cleanWName);
          const filteredSuggestions = data.results
            ...
            .filter((addr: string) => {
              ...
              return (
                normalizedAddr.includes(normalizedSelectedProvince) &&
                (!normalizedSelectedDistrict ||
                  normalizedAddr.includes(normalizedSelectedDistrict)) &&
                (!normalizedSelectedWard ||
                  normalizedAddr.includes(normalizedSelectedWard))
              );
            });
  ...
  const normalizedAddressPreview = (() => {
    ...
    return [addressDetail?.trim(), wardName, districtName, provinceName]
      .filter(Boolean)
      .join(", ");
  })();
  ...
  const isPhoneValid = Boolean(
    phoneValue?.trim() && phoneValue.replace(/\D/g, "").length === 10,
  );
    ...
    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-5 gap-y-6">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className="h-10 text-[13px] border-slate-200 rounded-[4px] shadow-none font-normal focus:border-emerald-500"
                />
                {errors.name && (
                  ...
                  render={({ field }) => (
                    <Popover open={openProvince} onOpenChange={setOpenProvince}>
                      <PopoverTrigger asChild>
                        ...
                        >
                          {field.value
                            ? provinces.find((p) => p.id === field.value)
                                ?.full_name
                      ...
                      <PopoverContent className="w-[--radix-popover-trigger-width] p-0 rounded-none">
                        <Command className="rounded-none">
                          ...
                          />
                          <CommandEmpty className="text-[12px] py-3 text-center">
                  ...
                  render={({ field }) => (
                    <Popover open={openWard} onOpenChange={setOpenWard}>
                      <PopoverTrigger asChild>
                        ...
                        >
                          {field.value
                            ? wards.find((w) => w.id === field.value)?.full_name
                            : "-- Chọn hoặc Gõ Phường/Xã --"}
                          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-[--radix-popover-trigger-width] p-0 rounded-none">
                ...
                <Label className="text-[10.5px] font-semibold text-slate-500 flex items-center justify-between">
                  ...
                  {isSearchingMap && (
                    <span className="text-blue-500 flex items-center gap-1">
                      <Loader2 size={10} className="animate-spin" /> Đang tìm...
                    </span>
                  )}
                </Label>
                <div className="relative">
                  <Input
                    ...
                    placeholder={
                      selectedProvince && selectedDistrict && selectedWard
                        ? "Gõ số nhà, tên đường để Geoapify gợi ý theo Tỉnh/Huyện/Xã..."
                        : "Vui lòng chọn Tỉnh/Thành, Quận/Huyện và Phường/Xã trước..."
                    }
                    ...
                    <ul className="absolute z-50 w-full bg-white border border-slate-200 shadow-lg mt-1 rounded-[4px] max-h-[250px] overflow-y-auto animate-in fade-in slide-in-from-top-1 custom-scrollbar">
                      {suggestions.map((addr, idx) => (
                        ...
                        >
                          <MapPinIcon
                            size={14}
                            className="mt-0.5 shrink-0 opacity-50 text-blue-500"
                          />
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-700 leading-relaxed">
                      {normalizedAddressPreview ||
                        "Chưa đủ dữ liệu để chuẩn hóa địa chỉ"}
                    </p>
                  </div>
                  {normalizedAddressPreview && (
                    <Button
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[999] border-t border-slate-200 bg-white px-4 py-3 lg:left-[260px] flex items-center justify-end gap-3">
        <Button
          type="button"
          variant="outline"
          className="min-w-[120px] h-10 text-[13px] font-medium border-slate-200 bg-white shadow-none hover:bg-slate-50 transition-all rounded-[4px]"
          onClick={() => router.back()}
        >
          Hủy
        </Button>
