...
export default function AddCustomerPage() {
  ...
  return (
    <form onSubmit={handleSubmit(onSave)} className="space-y-4 pb-[100px] bg-slate-50/30 p-4 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <div className="lg:col-span-9 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <Controller name="gender" control={control} render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value}>
                    <SelectTrigger className="h-[34px] text-[13px] border-[#ccc] rounded-none shadow-none focus:ring-0"><SelectValue /></SelectTrigger>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <Controller name="provinceId" control={control} render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value}>
                    <SelectTrigger className="h-[34px] text-[12px] border-[#ccc] rounded-none focus:ring-0 shadow-none"><SelectValue placeholder="-- Chọn Tỉnh/TP --" /></SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Controller name="districtId" control={control} render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value} disabled={!selectedProvince}>
                    <SelectTrigger className="h-[34px] text-[12px] border-[#ccc] rounded-none focus:ring-0 shadow-none"><SelectValue placeholder="-- Chọn Quận/Huyện --" /></SelectTrigger>
                    <SelectContent className="rounded-none max-h-[200px]">
                      {districts.map(d => <SelectItem key={d.id} value={d.id}>{d.full_name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                )} />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Controller name="wardId" control={control} render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value} disabled={!selectedDistrict}>
                    <SelectTrigger className="h-[34px] text-[12px] border-[#ccc] rounded-none focus:ring-0 shadow-none"><SelectValue placeholder="-- Chọn Phường/Xã --" /></SelectTrigger>
                    <SelectContent className="rounded-none max-h-[200px]">
                      {wards.map(w => <SelectItem key={w.id} value={w.id}>{w.full_name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                )} />
              </div>
              <div className="md:col-span-3 space-y-1.5">
                ...
                <Input {...register("addressDetail")} placeholder="Nhập địa chỉ chính xác..." className="h-[34px] text-[13px] border-[#ccc] rounded-none focus:border-blue-500 shadow-none" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#dcdcdc] p-6 rounded-none shadow-sm">
            ...
            <Textarea {...register("note")} placeholder="Nhập các đặc điểm lưu ý..." className="min-h-[120px] text-[13px] border-[#ccc] rounded-none focus-visible:ring-blue-500/20 shadow-none focus:border-blue-500" />
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-5">
          <div className="bg-white border border-[#dcdcdc] p-6 rounded-none shadow-sm">
            ...
            <Controller name="status" control={control} render={({ field }) => (
              <Select onValueChange={field.onChange} value={field.value}>
                <SelectTrigger className="h-[38px] text-[13px] border-[#ccc] rounded-none font-black text-emerald-600 shadow-none uppercase focus:ring-0"><SelectValue /></SelectTrigger>
