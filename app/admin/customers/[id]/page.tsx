...
import { 
  ...
  History, Info, ShoppingCart, CheckCircle2, 
  UserCircle, Wallet
} from "lucide-react";
...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isLoading) return (
    <div className="p-20 text-center flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-sm font-black uppercase text-slate-400 tracking-widest">Đang truy xuất hồ sơ khách hàng...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-4 pb-10">
      ...
      <div className="flex items-center gap-4 mb-2">
        <Button variant="ghost" size="icon" onClick={() => router.back()} className="h-8 w-8 text-slate-400 hover:text-blue-600">
          <ChevronLeft size={20} />
        </Button>
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <h1 className="text-[18px] font-black text-slate-800 uppercase tracking-tight">Chi tiết hồ sơ khách hàng</h1>
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-white border border-[#dcdcdc] rounded-none shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-50 flex flex-col items-center text-center">
              ...
              <h2 className="text-[16px] font-black text-slate-800 uppercase leading-tight mb-1">{customer?.name || "Chưa có tên"}</h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Mã định danh: {customer?.user?.id ? `USR-${customer.user.id}` : "KHÔNG CÓ TÀI KHOẢN"}</p>
              
              <div className="mt-6 grid grid-cols-2 gap-3 w-full">
                <div className="bg-emerald-50/50 p-3 border border-emerald-100 text-center">
                  <p className="text-[9px] font-bold text-emerald-600 uppercase mb-1 flex items-center justify-center gap-1"><Wallet size={10} /> Chi tiêu</p>
                  <p className="text-[14px] font-black text-emerald-700">0 ₫</p>
                </div>
                <div className="bg-blue-50/50 p-3 border border-blue-100 text-center">
                  <p className="text-[9px] font-bold text-blue-600 uppercase mb-1 flex items-center justify-center gap-1"><ShoppingCart size={10} /> Đơn hàng</p>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[13px] font-bold text-slate-700">{customer?.email || customer?.user?.email || "Chưa cập nhật"}</span>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[12px] font-medium text-slate-600 leading-snug">{customer?.addressDetail || "Chưa cập nhật địa chỉ cụ thể"}</span>
                </div>
              </div>
              <div className="pt-2 border-t border-slate-50 flex justify-between items-center">
                 <span className="text-[10px] font-black text-slate-400 uppercase">Trạng thái vận hành</span>
                 <span className={cn(
                 ...
                 )}>{customer?.status === "ACTIVE" ? "ĐANG HOẠT ĐỘNG" : "TẠM KHÓA"}</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <TabsContent value="history" className="mt-4">
              <div className="bg-white border border-[#dcdcdc] rounded-none shadow-sm overflow-hidden text-center p-20">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Hiện tại khách hàng chưa phát sinh đơn hàng</p>
                </div>
              </div>
            </TabsContent>
               ...
               <div className="bg-white border border-[#dcdcdc] rounded-none shadow-sm p-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                          <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">Tỉ lệ thanh toán đúng hạn</p>
                          <p className="text-[24px] font-black text-orange-700 leading-none">-- %</p>
                      </div>
                    </div>
                    <div className="text-right bg-emerald-50 px-4 py-2 border border-emerald-100">
                      ...
                      <p className="text-[13px] font-black text-emerald-700 uppercase">Đối tác mới</p>
                    </div>
                </div>
              </div>
            </TabsContent>
