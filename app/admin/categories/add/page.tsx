...
import { createCategory, getCategoryById, updateCategory, getCategories } from "@/app/services/CategoryService";
import {
  X, Settings, HelpCircle, Save,
  ChevronLeft, Tag, AlertCircle, Camera, Loader2
} from "lucide-react";
...
import { AdminCategorySchema, AdminCategoryForm } from "@/app/types/admin.schema";
...
export default function AddCategoryPage() {
      ...
      const fetchCategory = async () => {
        try {
          ...
          reset({
            ...
            parentId: (data.parentId && data.parentId !== 0) ? String(data.parentId) : "none"
          });
        } catch (error) {
          console.error(error);
  ...
  const handleThumbnailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        setValue("image", event.target?.result as string, { shouldValidate: true });
      };
          ...
          <div className="bg-white border border-[#dcdcdc] p-[15px_20px] rounded-[4px] shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
            ...
            <div className="grid grid-cols-1 gap-y-6">
              ...
              <div className="space-y-[2px] relative pb-5">
                <Label className="text-[11px] font-bold text-slate-500 uppercase tracking-tighter">Tên danh mục *</Label>
                <Input
                  {...register("name")}
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        <SelectItem value="none" className="font-bold text-slate-500">
          ...
          <div className="bg-white border border-[#dcdcdc] p-[15px_20px] rounded-[4px] shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
            ...
            <div
              ...
              className={cn(
                ...
                errors.image ? "border-red-300 bg-red-50" : "border-[#ddd] bg-[#fcfcfc] hover:bg-emerald-50 hover:border-emerald-300"
              )}
            >
              {thumbnailPreview ? (
                ...
                <div className="text-center">
                  <Camera size={32} className={cn("mx-auto mb-2", errors.image ? "text-red-300" : "text-slate-200")} />
                  <span className={cn("text-[9px] font-bold uppercase tracking-tighter", errors.image ? "text-red-400" : "text-slate-400")}>
                    Nhấp để tải ảnh
                  </span>
                </div>
              )}
            ...
            {errors.image && <p className="text-[10px] text-red-500 font-bold text-center mt-2 uppercase">{errors.image.message}</p>}
