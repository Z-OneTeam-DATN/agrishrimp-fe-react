...
export default function ProcessingPage() {
  ...
  const fetchOrders = useCallback(async (q?: string) => {
    setIsLoading(true);
    try {
      const data = await orderService.getBranchOrders("CONFIRMED", q || undefined);
      setOrders(data);
    } catch {
      toast.error("Không thể tải danh sách đơn hàng chờ xử lý.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleStartPacking = async (e: React.MouseEvent, orderId: number, orderCode: string) => {
    e.stopPropagation();
    try {
      await orderService.updateBranchOrderStatus(orderId, "PROCESSING");
      toast.success(`Đơn hàng ${orderCode} đã chuyển sang đóng gói!`);
  ...
  return (
    <div className="p-4 space-y-4 bg-slate-50 min-h-screen">
      <h1 className="text-[18px] font-bold text-slate-800">
        Chờ xử lý: <span className="font-normal text-slate-600">({orders.length} đơn)</span>
      </h1>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={7} className="h-32 text-center text-slate-500">Không có đơn hàng nào chờ xử lý.</TableCell></TableRow>
              ) : (
                orders.map((order) => {
                              ...
                              <div className="flex-1">
                                <div className="flex justify-between items-center mb-3">
                                  ...
                                  <Button
                                    ...
                                    onClick={(e) => handleStartPacking(e, order.orderId, order.orderCode)}
                                  >
                                    <PackageCheck size={14} className="mr-1.5" /> Bắt đầu đóng gói
                                  </Button>
                                </div>
