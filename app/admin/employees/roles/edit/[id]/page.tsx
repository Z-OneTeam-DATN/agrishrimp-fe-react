...
export default function EditRolePage() {
  ...
  const isSuperAdmin = normalizeRoleSlug(user?.role) === "SUPER_ADMIN";
  const isEditingSuperAdminRole = normalizeRoleSlug(roleSlug) === "SUPER_ADMIN";
  const canEditSystemPermissions =
    isSystemRole && isSuperAdmin && !isEditingSuperAdminRole;
  const canEditRole = !isSystemRole || canEditSystemPermissions;
  ...
  useEffect(() => {
    const fetchRoleDetails = async () => {
      try {
        ...
        setEnabledScreens(
          ALL_ROLE_MODULE_IDS.filter((id) => grantedCodes.has(id)),
        );
        setAdvancedPerms(
          ALL_ROLE_ACTION_IDS.filter((id) => grantedCodes.has(id)),
        );
      } catch (error) {
  ...
  return (
    <div className="space-y-3 text-slate-800">
      <div
        className={cn(
          "mt-2 mb-8 space-y-4",
          isSystemRole && !canEditSystemPermissions && "opacity-80",
        )}
      >
        ...
        <div className="bg-white border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-3 border-b bg-slate-50/50 flex items-center justify-between">
            ...
            {canEditRole && (
              ...
              >
                {enabledScreens.length === ALL_ROLE_MODULE_IDS.length
                  ? "Tắt tất cả"
                  : "Bật tất cả"}
              </Button>
            )}
                        ...
                        >
                          ...
                          >
                            ...
                            <div
                              ...
                              onClick={() =>
                                toggleModule(screen.id, !isEnabled)
                              }
                            >
                              <div
                                className={cn(
